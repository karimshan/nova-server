...
public final class MapAreas {
	...
	public static final boolean isAtArea(String areaName, Location tile) {
		return isAtArea(Misc.getNameHash(areaName), tile);
	}

	public static final boolean isAtArea(int areaNameHash, Location tile) {
		int[] coordsList = mapAreas.get(areaNameHash);
		if (coordsList == null)
			return false;
		int index = 0;
		while (index < coordsList.length) {
			if (tile.getZ() == coordsList[index]
	...
	public static final int getRandomAreaHash() {
		synchronized (lock) {
			while (true) {
				long id = Misc.getRandom(Integer.MAX_VALUE)
						+ Misc.getRandom(Integer.MAX_VALUE);
