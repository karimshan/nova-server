...
public final class NPCSpawns {
	...
	public static final void addRealSpawn(String username, int id, Location tile, boolean walks) {
		try {
			FileWriter fstream = new FileWriter("data/npcs/spawns.txt", true);
			final BufferedWriter out = new BufferedWriter(fstream);
			out.write("// " + NPCDefinition.get(id).name + ", " + NPCDefinition.get(id).combatLevel + ", added by: " + username);
			out.newLine();
			out.write(id + " - " + tile.getX() + " " + tile.getY() + " " + tile.getZ() + " "+walks);
			out.newLine();
			out.close();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static final void loadSpawns() {
		try {
			BufferedReader in = new BufferedReader(new FileReader("data/npcs/spawns.txt"));
			while (true) {
				...
				boolean walk = true;
				Location tile = new Location(Integer.parseInt(splitedLine2[0]), Integer.parseInt(splitedLine2[1]), Integer.parseInt(splitedLine2[2]));
				if (splitedLine2.length == 4) {
					walk = Boolean.parseBoolean(splitedLine2[3]);
					Game.spawnNPC(npcId, tile, walk, false);
				} else {
					Game.spawnNPC(npcId, tile, true, false);
				}
			}
			in.close();
	...
	public static boolean removeSpawn(NPC npc) throws Throwable {
		synchronized (lock) {
			...
			int id = npc.getId();
			Location tile = npc.getRespawnTile();
		...
		try {
			...
			while (true) {
				...
				if (splitedLine2.length == 5) {
					mapAreaNameHash = Misc.getNameHash(splitedLine2[3]);
	...
	public static final void loadNPCSpawns(int regionId) {
		...
		try {
			...
			while (buffer.hasRemaining()) {
				...
				Game.spawnNPC(npcId, new Location(x, y, plane),
	...
	private static final void addNPCSpawn(int npcId, int regionId,
			Location tile, int mapAreaNameHash,
