...
import org.nova.game.player.ClanChat;
import org.nova.game.player.HintIcon;
import org.nova.game.player.Interfaces;
...
public class PacketSender extends Encoder {
	private int ID;

	private Player player;

	public PacketSender(Session session, Player player) {
		super(session);
		this.player = player;
	}
	...
	public void sendInterfaceHoverMessage(String message, int inter, int button, int slot) {
		OutputStream stream = new OutputStream();
		stream.writePacketVarShort(145);
		stream.writeString(message);
		stream.writeInt(inter);
	...
	public void sendShopLoadMainStock(int shopId) {
		...
		stream.writeString("vg");
		stream.writeInt(shopId);
		stream.writeInt(93);
		stream.writeInt(25);
		stream.endPacketVarShort();
		session.write(stream);
	...
	public void sendOverlay(int inter) {
		sendInterface(true, player.interfaces().fullScreen() ? 746 : 548, player.interfaces().fullScreen() ? 10 : 8, inter);
	}

	public void sendRemoveOverlay() {
		closeInterface(player.interfaces().fullScreen() ? 746 : 548, player.interfaces().fullScreen() ? 10 : 8);
	}
	...
	public void sendHintIcon(HintIcon icon) {
		...
		if (icon.getTargetType() == 0)
			...
			if (icon.getTargetType() == 1 || icon.getTargetType() == 10) {
				...
				stream.writeShort(0);
				stream.skip(4);
			} else if ((icon.getTargetType() >= 2 && icon.getTargetType() <= 6)) {
				stream.writeByte(0); 
				stream.writeShort(icon.getCoordX());
				stream.writeShort(icon.getCoordY());
				stream.writeByte(icon.getDistanceFromFloor() * 4 >> 2);
				stream.writeShort(icon.getDistanceShown());
			}
			stream.writeShort(icon.getFollowOnFeet() ? 40497 : -1);
		}
		session.write(stream);
	...
	public void sendAccessMask(int set1, int set2, int interfaceId1, int childId1, int interfaceId2, int childId2) {
		...
		s.writePacket(113);
		s.writeInt(interfaceId2 << 16 | childId2);
		s.writeShortLE(set2);
		s.writeShortLE(set1);
		s.writeShortLE128(0);
		s.writeIntLE(interfaceId1 << 16 | childId1);
		session.write(s);
	}

	public void sendAccessMask(int set, int inter, int child,  int off, int len) {
		OutputStream s = new OutputStream().setId(113)
		.addInt(set) 
		.addLEShort(len)
		.addLEShort(off)
		.addLEShortA(ID++) 
	...
	public void sendItems(int interfaceId, int childId, int type, Item[] inventory) {
		...
		OutputStream s = new OutputStream().setIdVarShort(120)
		.addInt(main)
		.addShort(type)
		.addShort(inventory.length);
	...
	public void sendItems(int interfaceId, int childId, int type, int[] itemArray, int[] itemAmt) {
		...
		for (int i = 0; i < itemArray.length; i++) {
			if (itemAmt[i] > 254) {
				s.writeByteC((byte) 255);
				s.writeIntV1(itemAmt[i]);
			} else {
				s.writeByteC((byte) itemAmt[i]);
			}
			s.writeShortLE(itemArray[i]+1);
		}
		s.endPacketVarShort();
		session.write(s);
	}
	...
	public PacketSender setInterfaceConfig(int interfaceId, int childId, boolean set) {
		OutputStream stream = new OutputStream();
		stream.setId(3);
		stream.writeShort(0);
	...
	public void sendRemoveGroundItem(FloorItem item) {
		sendLocation(item.getLocation());
		int localX = item.getLocation().getLocalX(
				player.getLastLoadedMapRegionTile(), player.getMapSize());
		int localY = item.getLocation().getLocalY(
				player.getLastLoadedMapRegionTile(), player.getMapSize());
	...
	public void sendSpriteOnInter(int interfaceId, int componentId, int spriteId) {
		try {
			OutputStream stream = new OutputStream();
			stream.writePacket(2);
	...
	public void sendProjectile(Entity receiver, Location startTile,
		...
		int duration = (Misc.getDistance(startTile.getX(), startTile.getY(),
	...
	public void sendDestroyObject(GlobalObject object) {
		...
		stream.writeByteC((offsetX << 4) | offsetY); // the hash for coordinates, useless
		stream.writeByte((object.getType() << 2) + (object.getRotation() & 0x3));
		session.write(stream);
	...
	public void sendFriend(String username, String displayName, int world,
		...
		stream.writeByte(player.getFriendsIgnores().getRank(
				Misc.formatPlayerNameForProtocol(username)));
		stream.writeByte(0);
		if (putOnline) {
	...
	public void sendDynamicMapRegion(boolean wasAtDynamicRegion) {
		...
		int mapHash = Location.VIEWPORT_SIZES[player.getMapSize()] >> 4;
	...
	public void sendWindowsPane(int id, int type) {
		player.interfaces().setWindowsPane(id);
		OutputStream stream = new OutputStream(4);
	...
	public void sendGraphics(Graphics graphics, Object target) {
		...
		if (target instanceof Location) {
			Location tile = (Location) target;
			hash = tile.getZ() << 28 | tile.getX() << 14 | tile.getY()
					& 0x3fff | 1 << 30;
	...
	public void closeInterface(int windowComponentId) {
		closeInterface(
				player.interfaces().getTabWindow(windowComponentId),
				windowComponentId);
		player.interfaces().removeTab(windowComponentId);
	}
	...
	public void sendInterface(boolean clickThrough, int windowId,
		...
		if (!(windowId == 752 && (windowComponentId == 9 || windowComponentId == 12))) { // if
																							// chatbox
			if (player.interfaces().containsInterface(
					windowComponentId, interfaceId))
				closeInterface(windowComponentId);
			if (!player.interfaces().addInterface(windowId,
					windowComponentId, interfaceId)) {
				return;
			}
		}
	...
	public void sendMessage(String text) {
		if(player.isBot())
			System.out.println("[Bot Loader] \""+player.getDisplayName()+"\" received the message: "+text);
	...
	public void sendMessage(int type, String text, Player p) {
		...
		if ((maskData & 0x1) != 0) {
			...
			if (p.hasDisplayName())
				stream.writeString(Misc.formatPlayerNameForDisplay(p
						.getUsername()));
		}
		stream.writeString(text);
		stream.endPacketVarByte();
	...
	public void setGe(int slot, int progress, int item,
			int price, int amount, int currentAmount) {
		OutputStream output = new OutputStream();
		output.writePacket(61);
		output.writeByte(slot);
...
//		session.write(stream); // This works perfectly, but I prefer the method below more.
		player.interfaces().sendInterface(899);
	...
	public void sendAccessMask(Player player, int min, int max, int interfaceId, int childId, int hash) {
	 ...
	 public void sendRunScriptBlank(int scriptId) {
		...
		public void sendConfig1(int id, int value) {
			...
			stream.writeShort(id);
			stream.writeByte128(value);//try now
			session.write(stream);
		}
		...
		public void sendChatBoxInterface(int interfaceId) {
			sendInterface(true, 752, Interfaces.CHAT_BOX_TAB, interfaceId);
		}

		public void sendPlayerOnInterface(int inter, int button) {
			OutputStream stream = new OutputStream(5);
			stream.writePacket(114);
			stream.writeIntLE(inter << 16 | button);
			session.write(stream);
		}
		...
		public void itemOnInterface(int interfaceid, int componentId, int amount, int id) {
			OutputStream stream = new OutputStream(11);
			stream.writePacket(9);
			stream.writeShortLE(id);
			stream.writeInt(amount);
