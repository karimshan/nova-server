...
public final class GrabPacketsEncoder extends Encoder {
	...
	public final ChannelBuffer getArchivePacketData(int indexId, int archiveId,
		...
		try {
			if (indexId != 255) {
				if (archiveId > Cache.INSTANCE.getIndices()[indexId].getMainFile().getArchivesCount())
					return null;
				if (indexId > Cache.INSTANCE.getIndices().length)
					return null;
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
		byte[] archive = indexId == 255 ? Cache.INSTANCE.getIndex255()
				.getArchiveData(archiveId) : Cache.INSTANCE.getIndices()[indexId]
