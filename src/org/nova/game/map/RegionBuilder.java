...
public final class RegionBuilder {
	...
	public static void init() {
		...
		for (int mapX = 0; mapX < MAX_REGION_X; mapX++) {
			for (int mapY = 0; mapY < MAX_REGION_Y; mapY++) {
				if (Cache.INSTANCE.getIndices()[5].getArchiveId("m" + mapX + "_"
						+ mapY) != -1)
					EXISTING_MAPS.add(getRegionHash(mapX, mapY));
			}
		}
		Game.getRegion(7503, true);
		Game.getRegion(7759, true);
		/*
	 ...
	 * throws InterruptedException { for (int xn = x - ratio; xn < x + ratio;
	 * xn++) { for (int yn = y - ratio; yn < y + ratio; yn++) { if (Math.pow(2,
	 * x - xn) + Math.pow(2, y - yn) <= Math.pow(2, ratio)) { int regionId = new
	 * Location(xn, yn, 0).getRegionId(); Region region =
	 * World.getRegion(regionId); int baseLocalX = xn - ((regionId >> 8) * 64);
	 * int baseLocalY = yn - ((regionId & 0xff) * 64); while (region. != 2) { //
	 * blocks waiting // for load of // region to be come //
	 * System.out.println("nocliping: "+xn+", "+yn); Thread.sleep(1); }
	 * System.out.println("nocliping: " + xn + ", " + yn + ", " + baseLocalX +
	 * ", " + baseLocalY); System.out
	 * .println(region.forceGetRegionMap().getMasks
	 * ()[plane][baseLocalX][baseLocalY]);
	 * region.forceGetRegionMap().setMask(plane, baseLocalX, baseLocalY, 0);
	 * System.out
	 * .println(region.forceGetRegionMap().getMasks()[plane][baseLocalX
	 * ][baseLocalY]);
	 * 
	 * region.forceGetRegionMapClipedOnly().setMask(plane, baseLocalX,
	 * baseLocalY, 0); } } } } /
	 */
		...
		synchronized (ALGORITHM_LOCK) {
			for (int regionX = 1; regionX <= MAX_REGION_X - regionsDistanceX; regionX++) {
				skip: for (int regionY = 1; regionY <= MAX_REGION_Y
					...
					int regionHash = getRegionHash(regionX, regionY);
		...
		for (int xOffset = 0; xOffset < widthChunks; xOffset++) {
			for (int yOffset = 0; yOffset < heightChunks; yOffset++) {
				...
				for (int pIndex = 0; pIndex < toPlanes.length; pIndex++) {
					...
					Game.getRegion((((rx / 8) << 8) + (ry / 8)), true);
				}
			}
		}
	}
	...
	public static void copyChunk(int fromChunkX, int fromChunkY, int fromPlane,
		...
		toRegion.getRegionCoords()[toPlane][regionOffsetX][regionOffsetY][3] = rotation;
		Game.getRegion((((fromChunkY / 8) << 8) + (fromChunkX / 8)), true);
	}
	...
	public static DynamicRegion createDynamicRegion(int regionId) {
		synchronized (ALGORITHM_LOCK) {
			Region region = Game.getRegions().get(regionId);
			if (region != null) {
			...
			Game.getRegions().put(regionId, newRegion);
			return newRegion;
		}
	}
	...
	public static void destroyRegion(int regionId) {
		...
		if (region != null) {
			...
			Game.getRegions().remove(regionId);
	...
	private static boolean emptyRegion(int regionX, int regionY,
		...
		if (Cache.INSTANCE.getIndices()[5].getArchiveId("m" + rx + "_" + ry) != -1)
			return false; // a real map already exists
		Region region = Game.getRegions().get((rx << 8) + ry);
