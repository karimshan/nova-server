...
public final class LocalNPCUpdate {
	...
	private void processInScreenNPCs(OutputStream stream,
		...
		for (Iterator<NPC> it = localNPCs.iterator(); it.hasNext();) {
			...
			boolean needUpdate = (updateBlockData.getOffset()
					+ stream.getOffset() < (Constants.PACKET_SIZE_LIMIT - 500))
					&& n.needMasksUpdate();
			...
			if (walkUpdate) {
				...
				if (n.getNextRunDirection() != -1)
					stream.writeBits(1, 1);
				stream.writeBits(3,
						Misc.getNpcMoveDirection(n.getNextWalkDirection()));
				if (n.getNextRunDirection() != -1)
					stream.writeBits(3,
							Misc.getNpcMoveDirection(n.getNextRunDirection()));
	...
	private void addInScreenNPCs(OutputStream stream,
		...
		for (int regionId : player.getMapRegionsIds()) {
			List<Integer> indexes = Game.getRegion(regionId).getNPCsIndexes();
			if (indexes == null)
				continue;
			for (int npcIndex : indexes) {
				if (localNPCs.size() == Constants.LOCAL_NPCS_LIMIT
						|| (updateBlockData.getOffset() + stream.getOffset() > (Constants.PACKET_SIZE_LIMIT - 500)))
					break;
	...
	private void appendUpdateBlock(NPC n, OutputStream data, boolean added) {
		...
		if (n.getNextForceTalk() != null)
			maskData |= 0x2;
		if (n.getNextFaceLocation() != null && n.getNextRunDirection() == -1
		...
		if (n.getNextFaceLocation() != null)
			applyFaceLocationMask(n, data);
		if (n.getNextForceMovement() != null)
		...
		if(n.getBodyGlow() != null)
			applyBodyGlowMask(n, data);
		if(n.hasValidHoverMessage())
			applyHoverMessageMask(n, data);
	}
	
	private void applyHoverMessageMask(NPC n, OutputStream data) {
		data.writeString(n.getHoverMessage());
	}

	private void applyBodyGlowMask(NPC n, OutputStream data) {
		data.writeByte128(n.getBodyGlow().getColors()[0]);
		data.write128Byte(n.getBodyGlow().getColors()[1]);
		data.writeByte(n.getBodyGlow().getColors()[2]);
		data.write128Byte(n.getBodyGlow().getColors()[3]);
		data.writeShortLE(0); // Delay but it's weird so i'm going to leave it at 0
		data.writeShort(n.getBodyGlow().getDuration());
		n.setLastBodyGlowColors();
	}
	...
	private void applyFaceLocationMask(NPC n, OutputStream data) {
		data.writeShortLE(n.getNextFaceLocation().getX() * 2 + 1);
		data.writeShortLE(n.getNextFaceLocation().getY() * 2 + 1);
	}
