...
public final class LocalPlayerUpdate {
	...
	public boolean needAppearenceUpdate(int index, byte[] hash) {
		if (totalRenderDataSentLength > ((Constants.PACKET_SIZE_LIMIT - 500) / 2)
				|| hash == null)
			return false;
			...
			if (needsAdd(p)) {
				...
				if (p != null && hash != regionHashes[playerIndex]) {
					...
					for (int i2 = i + 1; i2 < outPlayersIndexesCount; i2++) {
						...
						Player p2 = Game.getPlayers().get(p2Index);
						if (needsAdd(p2)
	...
	private void processLocalPlayers(OutputStream stream,
		...
		for (int i = 0; i < localPlayersIndexesCount; i++) {
			...
			if (needsRemove(p)) {
				...
				regionHashes[playerIndex] = p.getLastLocation() == null ? p
						.get18BitsLocationHash() : p.getLastLocation()
						.get18BitsLocationHash();
				int hash = p.get18BitsLocationHash();
				if (hash == regionHashes[playerIndex])
					stream.writeBits(1, 0);
				...
				if (p.hasTeleported()) {
					...
					stream.writeBits(2, 3);
					int xOffset = p.getX() - p.getLastLocation().getX();
					int yOffset = p.getY() - p.getLastLocation().getY();
					int planeOffset = p.getZ() - p.getLastLocation().getZ();
					if (Math.abs(p.getX() - p.getLastLocation().getX()) <= 14 // 14
							&& Math.abs(p.getY() - p.getLastLocation().getY()) <= 14) { // 14
						stream.writeBits(1, 0);
	...
	private void applyBodyGlowMask(Player p, OutputStream data) {
		data.writeByte128(p.getBodyGlow().getColors()[0]);
		data.writeByte128(p.getBodyGlow().getColors()[1]);
		data.writeByteC(p.getBodyGlow().getColors()[2]);
		data.writeByte(p.getBodyGlow().getColors()[3]);
		data.writeShortLE128(0); // Delay but it's weird so i'm going to leave it at 0
		data.writeShort(p.getBodyGlow().getDuration());
		p.setLastBodyGlowColors();
	}
