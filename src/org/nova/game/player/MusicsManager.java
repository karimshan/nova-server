...
public final class MusicsManager implements Serializable {
	...
	public void setPlayer(Player player) {
		this.player = player;
		playingMusic = Game.getRegion(player.getRegionId()).getMusicId();
	}
	...
	public void refreshPlayListConfigs() {
		...
		for (int i = 0; i < PLAY_LIST_CONFIG_IDS.length; i++)
			player.packets().sendConfig(PLAY_LIST_CONFIG_IDS[i],
					configValues[i]);
	}
	...
	public void unlockMusicPlayer() {
		player.packets().sendUnlockIComponentOptionSlots(187, 1, 0,
		...
		if (playListOn && playList.size() > 0) {
			if (shuffleOn)
				playingMusic = playList
						.get(Misc.getRandom(playList.size() - 1));
			else {
			...
			playingMusic = unlockedMusics.get(Misc.getRandom(unlockedMusics
					.size() - 1));
		playMusic(playingMusic);
	}
	...
	public void reset() {
		...
		player.getMusicsManager().checkMusic(
				Game.getRegion(player.getRegionId()).getMusicId());
	}
	...
	public void playMusic(int musicId) {
		...
		player.packets().sendMusic(musicId, playingMusic == -1 ? 0 : 100,
				255);
		playingMusic = musicId;
		int musicIndex = (int) ClientScriptMap.getMap(1351).getKeyForValue(
				musicId);
