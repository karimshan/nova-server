...
import org.nova.game.player.AreaEffect;
...
public final class SlaughterFieldsControler extends Controller {
     ...
     */
    private static Location regionBase;

    /**
     * A list of players in the ffa zone.
     */
    public static List<Player> ffaPlayers = new ArrayList<Player>();

    /**
     * A list of players in the pure zone.
     */
    public static List<Player> purePlayers = new ArrayList<Player>();

    /**
     * A list of players in the dangerous zone.
     */
    public static List<Player> dangerousPlayers = new ArrayList<Player>();

    /**
     * A list of players in the no-melee zone.
     */
    public static List<Player> meleeDisabledPlayers = new ArrayList<Player>();

    /**
     * A list of players in the no-range zone.
     */
    public static List<Player> rangeDisabledPlayers = new ArrayList<Player>();

    /**
     * A list of players in the no-magic zone.
     */
    public static List<Player> magicDisabledPlayers = new ArrayList<Player>();
	 ...
	 * Pure-zone (south)
	 */
	t = new Location(3264, 2872, 0);
	MapBuilder.copyAllPlanesMap(t.getChunkX(), t.getChunkY(), newCoords[0] + 1 + 7, newCoords[1] + 1, 6, 6);
	/*
	 * No-melee zone (west)
	 */
	t = new Location(3352, 3872, 0);
	MapBuilder.copyAllPlanesMap(t.getChunkX(), t.getChunkY(), newCoords[0] + 3, newCoords[1] + 9, 5, 4);
	/*
	 * No-range zone (north)
	 */
	t = new Location(3328, 3904, 0);
	MapBuilder.copyAllPlanesMap(t.getChunkX(), t.getChunkY(), newCoords[0] + 7, newCoords[1] + 15, 8, 10);
	/*
	 * No-magic zone (east)
	 */
	t = new Location(3784, 3520, 0);
	MapBuilder.copyAllPlanesMap(t.getChunkX(), t.getChunkY(), newCoords[0] + 15, newCoords[1] + 8, 7, 8);
	regionBase = new Location(newCoords[0] << 3, newCoords[1] << 3, 0);
	/*
	 * Dangerous zone (north of no-range)
	 */
	t = new Location(3168, 3800, 0);
	MapBuilder.copyAllPlanesMap(t.getChunkX(), t.getChunkY(), newCoords[0] + 7, newCoords[1] + 30, 9, 7);
	/*
	 * Entrance to no-melee zone
	 */
	Game.spawnObject(new GlobalObject(13403, 10, 3, regionBase.getX() + 65, regionBase.getY() + 80, 0));
	Game.spawnObject(new GlobalObject(13397, 10, 3, regionBase.getX() + 65, regionBase.getY() + 82, 0));
	Game.spawnObject(new GlobalObject(8987, 10, 0, regionBase.getX() + 57, regionBase.getY() + 83, 0));
	/*
	 * Entrance to no-range zone
	 */
	Game.spawnObject(new GlobalObject(13397, 10, 0, regionBase.getX() + 96, regionBase.getY() + 110, 0));
	Game.spawnObject(new GlobalObject(15624, 10, 0, regionBase.getX() + 94, regionBase.getY() + 110, 0));
	Game.spawnObject(new GlobalObject(8987, 10, 0, regionBase.getX() + 96, regionBase.getY() + 124, 0));
	/*
	 * Entrance to no-magic zone
	 */
	Game.spawnObject(new GlobalObject(13403, 10, 1, regionBase.getX() + 109, regionBase.getY() + 103, 0));
	Game.spawnObject(new GlobalObject(15624, 10, 1, regionBase.getX() + 109, regionBase.getY() + 105, 0));
	Game.spawnObject(new GlobalObject(8987, 10, 0, regionBase.getX() + 121, regionBase.getY() + 102, 0));
	/*
	 * Entrance to pure zone
	 */
	Game.spawnObject(new GlobalObject(13403, 10, 2, regionBase.getX() + 80, regionBase.getY() + 65, 0));
	Game.spawnObject(new GlobalObject(13397, 10, 2, regionBase.getX() + 81, regionBase.getY() + 65, 0));
	Game.spawnObject(new GlobalObject(15624, 10, 2, regionBase.getX() + 82, regionBase.getY() + 65, 0));
	Game.spawnObject(new GlobalObject(8987, 10, 0, regionBase.getX() + 81, regionBase.getY() + 53, 0));
	/*
	 * Entrance to dangerous zone.
	 */
	Game.spawnObject(new GlobalObject(11354, 10, 0, regionBase.getX() + 88, regionBase.getY() + 95, 0));
	/*
	 * Exit of dangerous zone.
	 */
	Game.spawnObject(new GlobalObject(11354, 10, 0, regionBase.getX() + 78, regionBase.getY() + 248, 0));
	/*
	 * Exit the Slaughter fields.
	 */
	Game.spawnObject(new GlobalObject(8987, 10, 0, regionBase.getX() + 66, regionBase.getY() + 76, 0));

	/*
	 * Person who handles teleporting.
	 */
	Game.spawnNPC(14872, new Location(3089, 3498, 0), -1, true);
    }
	...
	player.setLocation(regionBase.transform(66, 74, 0));
	areaCount++;
	if (areaCount == 1)
	    initArea();
    }
	...
	player.packets().sendMessage("You cannot summon familiars in the slaughter fields");
	return false;
    }
	...
	if (isDangerousZone(player)) {
	    player.packets().sendMessage("You can't teleport in the dangerous zone.");
	    return false;
	}
	return true;
    }
	...
	if (player.getCombatDefinitions().getSpellId() > 0 && isNonMagicZone(player)) {
	    player.packets().sendMessage("Magic combat has been disabled in this area!", true);
	    return false;
	} else if (isRanging && isNonRangeZone(player)) {
	    player.packets().sendMessage("Ranged combat has been disabled in this area!", true);
	    return false;
	} else if (!isRanging && isNonMeleeZone(player) && player.getCombatDefinitions().getSpellId() <= 0) {
	    player.packets().sendMessage("Melee combat has been disabled in this area!", true);
	    return false;
	}
	if (isSafe(target)) {
	    player.packets().sendMessage("This player is in the safe zone.");
	    return false;
	} else if (isSafe(player)) {
	    player.packets().sendMessage("You can't attack players in the safe zone.");
	    return false;
	}
	...
	if (target instanceof Player) {
		...
		player.packets().sendMessage("That player is not in the dangerous part of the fields.");
		return false;
	    }
	    return true;
	}
	...
	if (!logout) {
		...
		player.packets().sendIComponentText(746, 17, "");
	    player.packets().sendIComponentText(548, 10, "");
	    player.packets().sendIComponentText(548, 11, "");
	...
	switch (object.getId()) {
		...
		if (object.matches(regionBase.transform(81, 53, 0))) {
		    ...
		    Magic.sendObjectTeleportSpell(player, false, new Location(3088, 3498, 0));
		    player.getControllerManager().forceStop();
		} else {
		    return true;
		}
     ...
     * @param o The object.
     */
    private static void handleMeleeTarget(final Player p, final GlobalObject o) {
	...
	switch (o.getRotation()) {
		...
		break;
	    case 2:
		if (p.getSkills().getLevelFromXP(Skills.DEFENCE) != 1) {
		    p.packets().sendMessage("You can only enter this area with 1 defence.");
		    return;
		}
	...
	if (PlayerCombat.isRanging(p) != 0) {
	    p.packets().sendMessage("You can't use a range weapon to hit this dummy.");
	    return;
	}
	...
	WorldTasksManager.schedule(new WorldTask() {
		...
		WorldTasksManager.schedule(new WorldTask() {
		    ...
		    public void run() {
			p.unlock();
			p.packets().sendMessage("You hit the dummy and hear a clicking sound.");
     ...
     * @param o The object.
     */
    private static void handleElementalBalance(final Player p, final GlobalObject o) {
	Location destination = null;
	switch (o.getRotation()) {
	    case 0:
		destination = regionBase.transform(96, 125, 0);
	...
	final Location dest = destination;
	p.lock();
	p.packets().sendObjectAnimation(o, new Animation(3583));
	WorldTasksManager.schedule(new WorldTask() {
	    @Override
	    public void run() {
		p.setNextGraphics(new Graphics(2728));
		p.setNextAnimation(new Animation(2791));
		p.packets().sendObjectAnimation(o, new Animation(3584));
		Game.sendProjectile(p, o, 2735, 18, 36, 50, 50, 3, 0);
		Game.sendProjectile(p, o, 2736, 18, 36, 50, 50, 20, 0);
		Game.sendProjectile(p, o, 2736, 18, 36, 50, 50, 110, 0);
		WorldTasksManager.schedule(new WorldTask() {
		    @Override
		    public void run() {
			p.unlock();
			p.packets().sendObjectAnimation(o, new Animation(3585));
			p.packets().sendGraphics(new Graphics(2741, 0, 165 << 2), o);
			p.packets().sendMessage("The elemental balance starts to glow as you heat it, and draws you to the other side.");
			Magic.sendObjectTeleportSpell(p, false, dest);
			register(p, o.getRotation());
		    }
		}, 1);
     ...
     * @param o The object.
     */
    private static void handleRangeTarget(final Player p, final GlobalObject o) {
	Location destination = null;
	int x = 0, y = 0;
	switch (o.getRotation()) {
	    case 1:
	...
	if (!o.transform(x, y, 0).matches(p)) {
	    p.packets().sendMessage("You can't fire the target from here.");
	    return;
	}
	if (PlayerCombat.isRanging(p) == 0) {
	    p.packets().sendMessage("You need a range weapon to shoot the target.");
	    return;
	}
	final Location dest = destination;
	p.lock();
	WorldTasksManager.schedule(new WorldTask() {
		...
		if (weapon.defs().getName().contains("handcannon")) {
		    p.packets().sendMessage("Shooting the target with a handcannon would destroy the target.");
		    return;
		}
		p.setNextAnimation(new Animation(PlayerCombat.getWeaponAttackEmote(weapon.getId(), 0)));
		if (weapon.defs().getName().contains("crossbow") || weapon.defs().getName().contains("c'bow")) {
		    Game.sendProjectile(p, o, 27, 38, 36, 41, 32, 5, 0);
		} else if (weapon.defs().getName().contains("knife") || weapon.defs().getName().contains("dart") || weapon.defs().getName().contains("axe") || weapon.defs().getName().contains("javelin")) {
		    p.setNextGraphics(new Graphics(PlayerCombat.getKnifeThrowGfxId(weapon.getId()), 0, 100));
		    Game.sendProjectile(p, o, PlayerCombat.getKnifeThrowGfxId(weapon.getId()), 41, 36, 41, 32, 15, 0);
		} else if (weapon.defs().getName().contains("rystal bow")) {
		    p.setNextGraphics(new Graphics(250, 0, 96));
		    Game.sendProjectile(p, o, 249, 41, 36, 41, 35, 0, 0);
		} else {
		    p.setNextGraphics(new Graphics(19, 0, 96));
		    Game.sendProjectile(p, o, 10, 41, 36, 20, 35, 16, 0);
		}
		WorldTasksManager.schedule(new WorldTask() {
		    @Override
		    public void run() {
			p.unlock();
			p.packets().sendMessage("Bulls-eye! A magical force teleports you to the other side...");
			Magic.sendObjectTeleportSpell(p, false, dest);
			register(p, o.getRotation());
		    }
		}, 1);
	...
	if (player.interfaces().isFullScreen()) {
	    player.packets().sendIComponentText(746, 17, "Level: " + level + "<br><col=ff0000>" + allowedCombat + "</col>");
	    return;
	}
	player.packets().sendIComponentText(548, 10, "Level: " + level);
	player.packets().sendIComponentText(548, 11, "<col=ff0000>" + allowedCombat + "</col>");
    }
     ...
     */
    private static int getPossibleCombat(Location w) {
	int combatHash = 0;
     ...
     */
    public static final boolean isPureZone(Location w) {
     ...
     * @return {@code True} if so.
     */
    public static final boolean isFfaZone(Location w) {
     ...
     * @return {@code True} if so.
     */
    public static final boolean isDangerousZone(Location w) {
     ...
     * @return {@code True} if so.
     */
    public static final boolean isNonRangeZone(Location w) {
     ...
     * @return {@code True} if so.
     */
    public static final boolean isNonMeleeZone(Location w) {
     ...
     * @return {@code True} if so.
     */
    public static final boolean isNonMagicZone(Location w) {
     ...
     * @return {@code True} if so.
     */
    public static final boolean isSafe(Location w) {
     ...
     * @return {@code True} if so.
     */
    public static boolean isMulti(Location w) {
	...
	if (p.getFamiliar() != null || p.getRealPet() != null) {
	    p.packets().sendMessage("You cannot bring followers into the Slaughter Fields.");
	    return true;
	}
	for (Item item : p.getInventory().getItems().getItems()) {
		...
		p.packets().sendMessage("You can't bring " + item.defs().getName() + " to the Slaughter fields.");
		return true;
	    }
	}
	...
	*/return false;
     ...
     */
    public static Location getRegionBase() {
	return regionBase;
    }
