...
public class WarriorsGuildControler extends Controller {
...
player.packets().sendMessage("<col=00ff00>Your time is about to run out. Please leave the cyclops room.", true);
}
...
sendInterfaces();
player.packets().sendMessage("<col=00ff00>Your tokens reduce by 10.", true);
tick = 0;
}
tick++;
}
...
player.packets().closeInterface(player.interfaces().isFullScreen() ? 11 : 8);
removeControler();
}
...
player.interfaces().sendTab(player.interfaces().isFullScreen() ? 11 : 8, 1057);
