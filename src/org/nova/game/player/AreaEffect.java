...
import org.nova.game.player.controlers.SlaughterFieldsControler;
...
public final class AreaEffect extends TimerTask {
	...
	final Location tile = p.transform(0, 0, 0);
	Game.sendProjectile(tile.transform(-2, -3, 0), p, 58, 120, 0, 50, 50, 0, 0);
	WorldTasksManager.schedule(new WorldTask() {
	    @Override
	    public void run() {
		Game.sendGraphics(p, new Graphics(659), tile);
		for (Player p : dangerous ? SlaughterFieldsControler.dangerousPlayers : SlaughterFieldsControler.rangeDisabledPlayers) {
			...
			p.packets().sendMessage("You've been hit by a meteor crash.");
	...
	p.packets().sendMessage("You get suffocated in a sandstorm.");
	...
	final Location tile = p.transform(0, 0, 0);
	Game.sendGraphics(p, new Graphics(884), tile);
	WorldTasksManager.schedule(new WorldTask() {
	    @Override
	    public void run() {
		for (final Player p : dangerous ? SlaughterFieldsControler.dangerousPlayers : SlaughterFieldsControler.magicDisabledPlayers) {
		    if (tile.withinDistance(p, 1)) {
			p.packets().sendCameraShake(3, 25, 50, 25, 50);
			p.packets().sendMessage("You get hit by an earthquake.");
			p.applyHit(new Hit(null, 90 + RANDOM.nextInt(70), HitLook.REGULAR_DAMAGE));
			WorldTasksManager.schedule(new WorldTask() {
			    @Override
			    public void run() {
				p.packets().sendStopCameraShake();
			    }
			}, 1);
		    }
		}
	...
	p.packets().sendMessage("You've been bitten by a scorpion.");
