...
public final class Pots {
	...
	private enum Effects {
		...
		ZAMORAK_BREW(Skills.ATTACK) {
			...
			public void extra(Player player) {
				toAdd = (player.getSkills().getLevelFromXP(Skills.ATTACK));
		...
		SANFEW_SERUM(Skills.ATTACK, Skills.STRENGTH, Skills.DEFENCE,
			...
			public void extra(Player player) {
				player.getPrayer().restorePrayer(
						(int) ((int) (player.getSkills().getLevelFromXP(
		...
		ANTI_FIRE() {
			...
			public void extra(final Player player) {
				...
				player.packets().sendMessage( "You are now immune to dragonfire.");
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						...
						if (!stop) {
							player.packets().sendMessage("<col=480000>Your antifire potion is about to run out...</col>");
							stop = true;
						} else {
							stop();
							player.packets().sendMessage("<col=480000>Your antifire potion has ran out...</col>");
						}		
					}
				}, 500, 100);
			}
			...
			public void extra(Player player) {
				...
				.restorePrayer(
						(int) ((int) (Math.floor(player.getSkills()
								.getLevelFromXP(Skills.PRAYER) * 2.5) + 70) * player
								.getAuraManager()
								.getPrayerPotsRestoreMultiplier()));
			}
		},
		...
		EXTREME_STR_POTION(Skills.STRENGTH) {
			...
			public boolean canDrink(Player player) {
				if (player.getControllerManager().getControler() instanceof Wilderness) {
					player.packets().sendMessage(
		...
		SUPER_PRAYER() {
			...
			public void extra(Player player) {
				player.getPrayer().setPrayerpoints(
						(int) ((int) (70 + (player.getSkills().getLevelFromXP(
	...
	public static boolean pot(Player player, Item item, int slot) {
		...
		if (player.isTransformed()) {
			player.sendMessage("You cannot drink any potions while you're in a form.");
			return false;
		}
	...
	public static void applyOverLoadEffect(Player player) {
	 ...
	 public static boolean mixPot(Player player, Item fromItem, Item toItem, int fromSlot, int toSlot) {
			if (fromItem.getId() == VIAL || toItem.getId() == VIAL) {
				...
				player.packets().sendMessage("You pour from one container into the other.", true);
				return true;
			    }
			    ...
			    player.packets().sendMessage("You split the potion between the two vials.", true);
			    return true;
			}
			...
			player.getInventory().refresh(toSlot);
			player.packets().sendMessage("You pour from one container into the other" + (pot.isFlask() && doses1 == 0 ? " and the glass shatters to pieces." : "."));
			return true;
		    }
		public int getMaxDoses() {
		    return id.length;
