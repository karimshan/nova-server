...
public class Foods {
	...
	SUMMER_PIE {
		...
		int realLevel = player.getSkills().getLevelFromXP(Skills.AGILITY);
	...
	GARDEN_PIE {
		...
		int realLevel = player.getSkills().getLevelFromXP(Skills.FARMING);
	...
	FISH_PIE {
		...
		int realLevel = player.getSkills().getLevelFromXP(Skills.FISHING);
	...
	SPICY_STEW_EFFECT {
		...
		if (Misc.random(100) > 5) {
		    ...
		    int realLevel = player.getSkills().getLevelFromXP(Skills.COOKING);
	...
	CABAGE_MESSAGE {
	    ...
	    public void effect(Object object) {
		Player player = (Player) object;
		player.packets().sendMessage("You don't really like it much.", true);
	    }
	},
	...
	ONION_MESSAGE {
	    ...
	    public void effect(Object object) {
		Player player = (Player) object;
		player.packets().sendMessage("It hurts to see a grown " + (player.getAppearance().isMale() ? "male" : "female") + " cry.");
	    }
	},
	...
	BEER_EFFECT {
	    ...
	    public void effect(Object object) {
		Player player = (Player) object;
		player.packets().sendMessage("You drink the beer. You feel slightly reinvigorated...");
		player.packets().sendMessage("...and slightly dizzy too.");
		...
		int realLevel = player.getSkills().getLevelFromXP(Skills.STRENGTH);
	...
	WINE_EFFECT {
	    ...
	    public void effect(Object object) {
		Player player = (Player) object;
		player.packets().sendMessage("You drink the wine. You feel slightly reinvigorated...");
		player.packets().sendMessage("...and slightly dizzy too.");
		player.heal(70);
		int level = player.getSkills().getLevel(Skills.ATTACK);
    ...
    public static boolean eat(final Player player, Item item, int slot) {
    	if (player.isTransformed()) {
    		player.sendMessage("You cannot eat food in a form.");
    		return false;
    	}
    	Food food = Food.forId(item.getId());
	if (food == null)
	    return false;
	if (player.getFoodDelay() > Misc.currentTimeMillis() || player.getPotDelay() > Misc.currentTimeMillis())
	    return true;
	if (!player.getControllerManager().canEat(food))
	    return true;
	String name = ItemDefinition.get(food.getId()).getName().toLowerCase();
	player.packets().sendMessage("You eat the " + name + ".");
	...
	player.applyHit(new Hit(player, food.getHeal() * 10 + food.getExtraHP() * 10, HitLook.HEALED_DAMAGE));
	player.refreshHitPoints();
	if (player.getHitpoints() > currentHP)
	    player.packets().sendMessage("It heals some health.");
