...
import org.nova.game.npc.dungeonnering.DungeonBoss;
import org.nova.game.player.Player;
import org.nova.game.player.Skills;
import org.nova.game.player.content.dungeoneering.DungeonConstants;
import org.nova.game.player.content.dungeoneering.rooms.HandledRoom;
...
public final class Dungeoneering {
	...
	public static Object getRandomValue(Object[] objectArray) {
		return objectArray[Misc.getRandom(objectArray.length - 1)];
	}
	...
	public static int getBossId(int numberNPCSonRoom,
		...
		int npcId = -1;
		for (int id = 0; id < Misc.getNPCsSize(); id++) {
			NPCDefinition npcDef = NPCDefinition.get(id);
			if (npcDef.name != null && npcDef.name.equals(npcName)) {
				...
				if (lastCombatLevelDifference == -1
						...
						|| (difference == lastCombatLevelDifference && NPCDefinition
								.get(npcId).combatLevel < npcDef.combatLevel)) {
	...
	public static void main(String[] args) { // Test
		long start = Misc.currentTimeMillis();
		DungeonStructure smallDungeon = generateDungeonStructure(FROZEN_FLOORS,
				1, 120);
		System.out.println("Took: " + (Misc.currentTimeMillis() - start)
				+ " ms.");
	...
	public static class Dungeon {
		...
		private Dungeon(final int floor, final int complexity, final int size,
			...
			for (Player player : teamArray) {
				team.add(player);
				player.getControllerManager().startController("DungControler",
						this);
			}
			...
			CoresManager.slowExecutor.execute(new Runnable() {
				...
				public void run() {
					try {
						...
						Location base = getBaseCoords(structure
								.getBossRoomPos());
						MapAreas.addArea(
								dungeonBossRoomHash,
								new int[] { base.getZ(), base.getX() - 8,
		...
		public void startDungeon() {
			...
			Location homeTile = getHomeTeleTile();
			int[] homeRoom = structure.getStartRoomPos();
			Game.spawnNPC(11226, new Location(homeTile, 2), -1, true);
			for (Player player : team) {
				...
				player.setLocation(homeTile);
				playMusic(player, homeRoom);
				player.getCombatDefinitions().setSpellBook(3); // sets dungeon
																// spellbook
				player.packets().sendMessage("");
				player.packets().sendMessage("-Welcome to Daemonheim-");
				player.packets().sendMessage(
						"Floor " + floor + " Complexity " + 6 + " (Full)");
				player.packets().sendMessage("Dungeon Size: " + "Small");
				player.packets().sendMessage(
		...
		public Location getHomeTeleTile() {
			return getBaseCoords(structure.getStartRoomPos());
		}

		public Location getBaseCoords(int[] pos) {
			return new Location(((mapBaseCoords[0] << 3) + pos[0] * 16) + 8,
		...
		public int[] getCurrentRoomCoords(Location tile) {
			return new int[] { tile.getChunkX() << 3, tile.getChunkY() << 3 };
		}

		public int[] getCurrentRoomPos(Location tile) {
		...
		public int getLowestPartDungeonneringLevel() {
			...
			for (Player player : team) {
				int dungLevel = player.getSkills().getLevelFromXP(
		...
		public void openStairs() {
			...
			case FROZEN_FLOORS:
			default:
				object = new GlobalObject(3784, 10, 3,
						((mapBaseCoords[0] << 3) + pos[0] * 16) + 7,
						((mapBaseCoords[1] << 3) + pos[1] * 16) + 15, 0);
				break;
			}
			Game.spawnObject(object, false);
			for (Player player : team)
