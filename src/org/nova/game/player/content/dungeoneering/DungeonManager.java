...
import org.nova.game.npc.dungeonnering.AsteaFrostweb;
import org.nova.game.npc.dungeonnering.Glutenus;
import org.nova.game.npc.dungeonnering.Guardian;
import org.nova.game.npc.dungeonnering.Tokash;
...
public class DungeonManager {
	...
	public void enterRoom(Player player, int x, int y, GlobalObject obj) {
		...
		if (visibleMap[x][y] != null) {
			...
			player.setLocation(new Location(player.getX() + xOffset * 3,
	...
	public void telePartyToRoom(RoomReference reference) {
		...
		for (DungeonPartyPlayer dplayer : getParty().getTeam()) {
			dplayer.getPlayer().setLocation(tile);
	...
	public void linkPartyToDungeon() {
		for (DungeonPartyPlayer dplayer : getParty().getTeam()) {
			Player player = dplayer.getPlayer();
			player.getControllerManager().startController("DungeonControler",
					this);
			player.stopAll();
			player.reset();
	...
	public void spawnRandomNPCS(RoomReference reference) {
		...
		spawnNPC(reference, DungeonUtils.getGuardianCreature(floorType),
				2 + Misc.getRandom(13), 2 + Misc.getRandom(13), true, true);
		spawnNPC(reference, DungeonUtils.getGuardianCreature(floorType),
				2 + Misc.getRandom(13), 2 + Misc.getRandom(13), true, true);
		spawnNPC(reference, DungeonUtils.getGuardianCreature(floorType),
				2 + Misc.getRandom(13), 2 + Misc.getRandom(13), true, true);
		...
		spawnNPC(reference, DungeonUtils.getPassiveCreature(floorType),
				2 + Misc.getRandom(13), 2 + Misc.getRandom(13), true, false);
		spawnNPC(reference, DungeonUtils.getPassiveCreature(floorType),
				2 + Misc.getRandom(13), 2 + Misc.getRandom(13), true, false);
		spawnNPC(reference, DungeonUtils.getGuardianCreature(floorType),
				2 + Misc.getRandom(13), 2 + Misc.getRandom(13), true, true);
	...
	public void stairs(RoomReference reference, int x, int y) {
			...
			GlobalObject object;
			object = new GlobalObject(3784, 10, rotation+3,
	...
	public void spawnGlut(RoomReference reference, int id, int x, int y,
		...
		Location tile = new Location(
				...
				((boundChuncks[1] << 3) + reference.getY() * 16) + y, 0);
		if (check
				&& !Game.canMoveNPC(0, tile.getX(), tile.getY(),
						NPCDefinition.get(id).size))
			return;
	...
	public void spawnNPC(RoomReference reference, int id, int x, int y,
		...
		if (check
				&& !Game.canMoveNPC(0, tile.getX(), tile.getY(),
						NPCDefinition.get(id).size))
			return;
		NPC n = guardian ? new Guardian(id, tile, this, reference) : Game
