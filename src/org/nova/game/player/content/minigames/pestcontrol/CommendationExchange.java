...
public class CommendationExchange {//1875 TODO
	...
	player.interfaces().sendInterface(INTERFACE);
	...
	if (currentPoints - price < 0) {
	    player.packets().sendMessage("You don't have enough Commendations remaining to complete this exchange.");
	    return false;
	}
	...
	if (componentId == 68) {
	    ...
	    player.packets().sendHideIComponent(INTERFACE, 69, false);
	} else if (componentId == 75) {
	    player.packets().sendHideIComponent(INTERFACE, 70, true);
	    player.packets().sendHideIComponent(INTERFACE, 69, false);
	} else if (componentId == 20 || componentId == 73) {
	    openExchangeShop(player);
	} else if (componentId == 24 || componentId == 31) {
	    player.packets().sendHideIComponent(INTERFACE, 70, false);
	    player.packets().sendHideIComponent(INTERFACE, 69, true);
	} else if (componentId == 291) {
	    if (player.getSkills().getLevelFromXP(Skills.HERBLORE) < 25) {
		player.packets().sendMessage("You need a herblore level of 25 in order to purchase a herblore pack.");
		return;
	    } else if (!exchangeCommendation(player, 30))
		return;
	    player.getInventory().addItemDrop(Herbs.values()[Misc.random(5)].getHerbId() + 1, Misc.random(4), player.getLocation());
	    player.getInventory().addItemDrop(Herbs.values()[Herbs.values().length - 1].getHerbId() + 1, Misc.random(2), player.getLocation());
	    player.packets().sendMessage("You exchange 30 commendation points for a herblore pack.");
	} else if (componentId == 302) {
	    if (player.getSkills().getLevelFromXP(Skills.MINING) < 25) {
		player.packets().sendMessage("You need a mining level of 25 in order to purchase a herblore pack.");
		return;
	    } else if (!exchangeCommendation(player, 15))
		return;
	    player.getInventory().addItemDrop(441, Misc.random(20), player.getLocation());
	    player.getInventory().addItemDrop(454, Misc.random(30), player.getLocation());
	    player.packets().sendMessage("You exchange 15 commendation points for a mineral pack.");
	} else if (componentId == 313) {
	    if (player.getSkills().getLevelFromXP(Skills.FARMING) < 25) {
		player.packets().sendMessage("You need a farming level of 25 in order to purchase a herblore pack.");
		return;
	    } else if (!exchangeCommendation(player, 15))
		return;
	    player.packets().sendMessage("You exchange 15 commendation points for a seed pack.");
	} else {
	...
	player.getInventory().addItemDrop(itemId, rate, player.getLocation());
	player.packets().sendMessage("You exchange " + rate * 2 + " Commendations for a charm.");
    }
	...
	if (player.getSkills().getLevelFromXP(skill) < 25) {
	    player.packets().sendMessage("You need a " + Skills.SKILL_NAME[skill] + " of at least 25 in order to gain experience.");
	    return;
	}
	for (int i = 0; i < rate; i++) {
	...
	player.getMatrixDialogues().startDialogue("SimpleMessage", "You gain " + Misc.getFormattedNumber((int) experience) + " experience in " + Skills.SKILL_NAME[skill] + ".");
    }
	...
	if (!player.getSkills().hasRequiriments(Skills.ATTACK, 42, Skills.STRENGTH, 42, Skills.DEFENCE, 42, Skills.HITPOINTS, 42, Skills.RANGE, 42, Skills.MAGIC, 42, Skills.PRAYER, 22)) {
	    player.packets().sendMessage("You need an attack, strength, defence, constitution, range, and magic level of 42, and a prayer level of 22 in order to purchase void equipment.");
	    return;
	}
	...
	player.getInventory().addItemDrop(voidItem, 1, player.getLocation());
	player.getMatrixDialogues().startDialogue("ItemMessage", "You exchange " + cost + " commendation points for a " + ItemDefinition.get(voidItem).name.toLowerCase() + ".", voidItem);
    }
