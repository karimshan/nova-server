...
public class War extends TimerTask {
	...
	public void onClick(Player player, Player other, int button, int slotId) {
		...
		case 122:
			...
			return;
		case 109:
			player.packets().sendMessage("This is not added.");
			return;
		case 141:
			if (slotId == 3)
		...
		case 143:
			...
			player.packets().sendConfigByFile(5293, 1);
			other.packets().sendMessage("Other player has accepted.", true);
	...
	private void sendConfigs(Player player, Player other, int config, int value) {
		player.packets().sendConfigByFile(config, value);
		other.packets().sendConfigByFile(config, value);
	}
	...
	private void commenceWar(Player player, Player other) {
		...
		for (Player players : player.getCurrentFriendChat().getPlayers()) {
			if (players != player) {
				players.packets().sendMessage("<col=ff0000>Your clan has been challenged to a clan war!", true);
				players.packets().sendMessage("<col=ff0000>Step through the purple portal in the Challenge Hall.", true);
				players.packets().sendMessage("<col=ff0000>Battle will commence in 2 minutes.", true);
			}
		}
	...
	public void startControler(Player player) {
		player.getControllerManager().startController("ClanWarsControler", this);
	}
	...
	public void run() {
		...
		if (countdown == 0) {
			for (Player players : challengerTeam)
				players.packets().sendMessage("The war has been started.");
			setCanCommence(true);
		}
	...
	private void updateScreen() {
		for (Player players : challengerTeam)
			players.packets().sendIComponentText(265, 16, "" +countdown + "s");
	...
	private void endWar() {
		if (challengerTeam.size() < opponentTeam.size()) {
			for (Player players : opponentTeam) {
				players.packets().sendMessage("Your clan won.");
				players.getControllerManager().startController("ClanReqControler");
			}
