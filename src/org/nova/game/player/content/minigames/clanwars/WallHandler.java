...
public final class WallHandler {
	...
	public static void loadWall(ClanWars c) {
		Location base = c.getBaseLocation();
		List<GlobalObject> objects = new ArrayList<GlobalObject>();
		switch (c.getAreaType()) {
		case CLASSIC_AREA: //TODO: Real classic area.
			int objectOffset = 0;
			for (int x = 5; x < 54; x++) {
				objectOffset = (objectOffset + 1) % 3;
				GlobalObject object = new GlobalObject(28174 + objectOffset, 10, 0, base.getX() + x, base.getY() + 64, 0);
		...
		case PLATEAU:
			for (int x = 32; x < 84; x++) {
				GlobalObject object = new GlobalObject(38687, 10, 0, base.getX() + x, base.getY() + 32, 0);
				Game.spawnObject(object, true);
				objects.add(object);
			}
			break;
		case FORSAKEN_QUARRY:
			for (int x = 5; x < 25; x++) {
				GlobalObject object = new GlobalObject(38685, 10, 0, base.getX() + x, base.getY() + 33, 0);
				Game.spawnObject(object, true);
				objects.add(object);
			}
			for (int x = 31; x < 33; x++) {
			...
			break;
		case BLASTED_FOREST:
			GlobalObject object = new GlobalObject(38689, 10, 0, base.getX() + 4, base.getY() + 33, 0);
			Game.spawnObject(object, true);
			objects.add(object);
			for (int x = 5; x < 46; x++) {
				if (Game.getMask(0, base.getX() + x, base.getY() + 33) != 0) {
					continue;
				}
				object = new GlobalObject(38689, 10, 0, base.getX() + x, base.getY() + 33, 0);
				Game.spawnObject(object, true);
				objects.add(object);
			}
			for (int x = 55; x < 57; x++) {
			...
			object = new GlobalObject(38689, 10, 0, base.getX() + 59, base.getY() + 33, 0);
			Game.spawnObject(object, true);
			objects.add(object);
			break;
		case TURRETS:
			for (int x = 3; x < 60; x++) {
				object = new GlobalObject(38691, 10, 1, base.getX() + x, base.getY() + 64, 0);
				Game.spawnObject(object, true);
				objects.add(object);
			}
			break;
	...
	public static void dropWall(ClanWars clanWars) {
		List<GlobalObject> objects = clanWars.getWallObjects();
		Animation animation;
		...
		for (GlobalObject object : objects) {
			for (Player p : clanWars.getFirstPlayers()) {
				p.packets().sendObjectAnimation(object, animation);
			}
