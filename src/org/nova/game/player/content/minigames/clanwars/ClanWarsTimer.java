 ...
 *
 */
public final class ClanWarsTimer extends TimerTask {//this is the class for updating the timer
	...
	public void run() {
		...
		if (!started) {
			if (startTicks-- == 8) {
				...
				for (Player player : clanWars.getFirstPlayers()) {
					player.packets().sendGlobalConfig(270, 0);
					player.packets().sendGlobalConfig(260, 1);
				}
		...
		if (clanWars.getFirstPlayers().isEmpty() || clanWars.getSecondPlayers().isEmpty()) {
			if (++timeOut == 1_000) { //10 minutes until the war is time-out.
				clanWars.endWar();
				cancel();
				return;
			}
		...
		if ((timeLeft * 0.6) / 60 != lastMinutes) {
			...
			for (Player player : clanWars.getFirstPlayers()) {
				player.packets().sendGlobalConfig(270, lastMinutes);
			}
			for (Player player : clanWars.getSecondPlayers()) {
				player.packets().sendGlobalConfig(270, lastMinutes);
			}
	...
	public void refresh(Player p, boolean firstTeam) {
		p.packets().sendGlobalConfig(261, (firstTeam ? clanWars.getFirstPlayers() : clanWars.getSecondPlayers()).size());
		p.packets().sendGlobalConfig(262, (firstTeam ? clanWars.getSecondPlayers() : clanWars.getFirstPlayers()).size());
		p.packets().sendGlobalConfig(263, clanWars.getKills() >> (firstTeam ? 0 : 24) & 0xFFFF);
		p.packets().sendGlobalConfig(264, clanWars.getKills() >> (firstTeam ? 24 : 0) & 0xFFFF);
		p.packets().sendGlobalConfig(260, started ? 1 : 0);
		p.packets().sendGlobalConfig(270, started ? lastMinutes : startTicks);
	}
