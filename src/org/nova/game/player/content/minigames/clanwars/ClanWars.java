...
public final class ClanWars implements Serializable {
	 ...
	 */
	private transient List<GlobalObject> wallObjects;
	...
	public void switchRule(Rules rule, Player player) {
		...
		if (rule == Rules.NO_MAGIC) {
			if (get(Rules.NO_RANGE) && get(Rules.NO_MELEE)) {
				player.packets().sendMessage("You can't activate all combat style rules, how would you fight?");
				return;
			} else {
	...
	public static void sendConfig(Player player, Player other, int configId, int value) {
		...
		if (resetAccept) {
			player.packets().sendConfigByFile(5293, 0);
			other.packets().sendConfigByFile(5293, 0);
		}
		player.packets().sendConfigByFile(configId, value);
		other.packets().sendConfigByFile(configId, value);
	}
	...
	private void sendVictoryConfiguration(Player p) {
		switch (victoryType) {
		case -1:
			p.packets().sendConfigByFile(5280, 0);
			break;
		case 25:
			p.packets().sendConfigByFile(5280, 1);
			break;
		case 50:
			p.packets().sendConfigByFile(5280, 2);
			break;
		case 100:
			p.packets().sendConfigByFile(5280, 3);
			break;
		case 200:
			p.packets().sendConfigByFile(5280, 4);
			break;
		case 400:
			p.packets().sendConfigByFile(5280, 5);
			break;
		case 750:
			p.packets().sendConfigByFile(5280, 6);
			break;
		case 1_000:
			p.packets().sendConfigByFile(5280, 7);
			break;
		case 2_500:
			p.packets().sendConfigByFile(5280, 8);
			break;
		case 5_000:
			p.packets().sendConfigByFile(5280, 9);
			break;
		case 10_000:
			p.packets().sendConfigByFile(5280, 10);
			break;
		case -2:
			p.packets().sendConfigByFile(5280, 15);
			break;
		}
	}
	...
	private void sendTimeConfiguration(Player p) {
		switch (timeLeft) {
		case 500:
			p.packets().sendConfigByFile(5281, 1);
			break;
		case 1_000:
			p.packets().sendConfigByFile(5281, 2);
			break;
		case 3_000:
			p.packets().sendConfigByFile(5281, 3);
			break;
		case 6_000:
			p.packets().sendConfigByFile(5281, 4);
			break;
		case 9_000:
			p.packets().sendConfigByFile(5281, 5);
			break;
		case 12_000:
			p.packets().sendConfigByFile(5281, 6);
			break;
		case 15_000:
			p.packets().sendConfigByFile(5281, 7);
			break;
		case 18_000:
			p.packets().sendConfigByFile(5281, 8);
			break;
		case 24_000:
			p.packets().sendConfigByFile(5281, 9);
			break;
		case 30_000:
			p.packets().sendConfigByFile(5281, 10);
			break;
		case 36_000:
			p.packets().sendConfigByFile(5281, 11);
			break;
		case 48_000:
			p.packets().sendConfigByFile(5281, 12);
			break;
		case -1:
			p.packets().sendConfigByFile(5281, 0);
			break;
		}
	}
	...
	public void sendInterface(Player p, Player other) {
		...
		p.interfaces().sendInterface(791);
		p.packets().sendUnlockIComponentOptionSlots(791, 141, 0, 63, 0);
		p.packets().sendConfigByFile(5291, 0);
		p.packets().sendConfigByFile(5292, 0);
		p.packets().sendConfigByFile(5293, 0);
	}
			...
			CoresManager.slowExecutor.submit(new Runnable() {
				...
				public void run() {
					...
					for (Player p : firstTeam.getPlayers()) {
						if (p != player && p != other) {
							p.packets().sendMessage("<col=FF0000>Your clan has been challenged to a clan war!</col>");
							p.packets().sendMessage("<col=FF0000>Step through the purple portal in the Challenge Hall.</col>");
							p.packets().sendMessage("<col=FF0000>Battle will commence in 2 minutes.</col>");
						}
					}
					...
					baseLocation = new Location(newCoords[0] << 3, newCoords[1] << 3, 0);
	...
	public static void enter(Player p) {
		...
		p.packets().sendGlobalConfig(271, hasWar ? 1 : 0);
		p.interfaces().sendTab(p.interfaces().isFullScreen() ? 10 : 19, 265);
		if (hasWar && c.timer.isStarted() && c.isKnockOut()) {
			hasWar = false;
			p.packets().sendMessage("The war has already started!");
		...
		if (hasWar) {
			if (c.get(Rules.NO_FAMILIARS) && p.getFamiliar() != null) {
				p.packets().sendMessage("You can't enter the clan war with a familiar.");
				return;
			}
			...
			if (c.firstTeam == p.getCurrentFriendChat()) {
				p.setLocation(c.baseLocation.transform(c.areaType.getFirstSpawnOffsetX(), c.areaType.getFirstSpawnOffsetY(), 0));
				c.firstPlayers.add(p);
				c.timer.refresh(p, true);
			} else {
				Location northEast = c.baseLocation.transform(c.areaType.getNorthEastTile().getX() - c.areaType.getSouthWestTile().getX(), 
						c.areaType.getNorthEastTile().getY() - c.areaType.getSouthWestTile().getY(), 0);
				p.setLocation(northEast.transform(c.areaType.getSecondSpawnOffsetX(), c.areaType.getSecondSpawnOffsetY(), 0));
		...
		if (prefix == null || prefix == 0) {
			...
			p.setLocation(c.baseLocation.transform(c.areaType.getFirstDeathOffsetX(), c.areaType.getFirstDeathOffsetY(), 0));
		} else {
			c.timer.refresh(p, false);
			c.secondViewers.add(p);
			Location northEast = c.baseLocation.transform(c.areaType.getNorthEastTile().getX() - c.areaType.getSouthWestTile().getX(), 
					c.areaType.getNorthEastTile().getY() - c.areaType.getSouthWestTile().getY(), 0);
			p.setLocation(northEast.transform(c.areaType.getSecondDeathOffsetX(), c.areaType.getSecondDeathOffsetY(), 0));
		}
	}
	...
	public void leave(Player p, boolean ingame) {
		...
		p.packets().closeInterface(p.interfaces().isFullScreen() ? 10 : 19, 265);
		p.setLocation(new Location(2992, 9676, 0));
		...
		p.packets().closeInterface(resized ? 746 : 548, resized ? 10 : 19);
		updateWar();
	}
	...
	public void endWar() {
		...
		Location target = new Location(2992, 9676, 0);
		int firstType;
		int secondType;
		if (timer.isTimeOut()) {
			firstType = 1;
		...
		for (Player player : firstPlayers) {
			...
			player.packets().closeInterface(resized ? 746 : 548, resized ? 1 : 11);
			player.interfaces().sendInterface(790);
			player.packets().sendGlobalConfig(268, firstType);
		...
		for (Player player : secondPlayers) {
			...
			player.packets().closeInterface(resized ? 746 : 548, resized ? 1 : 11);
			player.interfaces().sendInterface(790);
			player.packets().sendGlobalConfig(268, secondType);
			player.getControllerManager().startController("clan_wars_request");
			player.setForceMultiArea(true);
			player.stopAll(true, false);
			player.reset();
		...
		for (Player p : viewers) {
			...
			boolean resized = p.interfaces().isFullScreen();
			p.packets().closeInterface(resized ? 746 : 548, resized ? 1 : 11);
			p.getControllerManager().startController("clan_wars_request");
			p.setForceMultiArea(true);
		}
		...
		for (Player player : firstTeam.getPlayers()) {
			player.packets().sendMessage(firstMessage);
		}
		for (Player player : secondTeam.getPlayers()) {
			player.packets().sendMessage(secondMessage);
		}
	 ...
	 */
	public List<GlobalObject> getWallObjects() {
		return wallObjects;
	}
	 ...
	 */
	public void setWallObjects(List<GlobalObject> wallObjects) {
