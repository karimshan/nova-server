...
public final class RequestController extends Controller {
	...
	@Override
	public void start() {
		player.packets().sendPlayerOption("Challenge", 1, false);
		sendInterfaces();
		...
		player.setCoordsEvent(new CoordsEvent(target, new Runnable() {
			...
			public void run() {
				...
				if (target == null || target.hasFinished() || !target.withinDistance(player, 14)
						|| !(target.getControllerManager().getControler() instanceof RequestController)) {
					player.packets().sendMessage("Unable to find " 
						+ (target == null ? "your target" : target.getDisplayName()));
					return;
				}
				if (target.interfaces().containsScreenInter()) {
				...
				target.packets().sendClanWarsRequestMessage(player);
				player.packets().sendMessage("Sending challenge request...");
			}			
		}, player.getSize()));
		return true;
	}
	...
	public boolean processObjectClick1(GlobalObject object) {
		...
		case 28213:
			if (player.getCurrentFriendChat() == null || player.getCurrentFriendChat().getClanWars() == null) {
				player.getMatrixDialogues().startDialogue("clan_wars_view");
				return false;
			}
			ClanWars.enter(player);
			return false;
		case 38698: //Safe FFA portal.
			player.setLocation(new Location(2815, 5511, 0));
			player.getControllerManager().startController("clan_wars_ffa", false);
			return false;
		case 38699: //Risk FFA portal.
			player.setLocation(new Location(3007, 5511, 0));
	...
	public void stop() {
		...
		player.packets().closeInterface(
				player.interfaces().isFullScreen() ? 1 : 11);
		player.packets().sendPlayerOption("null", 1, false);
	}
	...
	private static boolean canRequest(Player player, Player target, boolean message) {
		...
		if (message && text != null) {
			player.packets().sendMessage(text);
		}
		return text == null;
	}
