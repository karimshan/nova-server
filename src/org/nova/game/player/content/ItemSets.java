...
public class ItemSets {
	...
	Sets set = getSet(id);
	if (set == null) {
	    player.packets().sendMessage("This isn't a set item, you can't break it up into component parts.");
	    return;
	}
	if (player.getInventory().getFreeSlots() < set.items.length - 1) {
	    player.packets().sendMessage("You don't have enough inventory space for the component parts.");
	    return;
	}
	player.getInventory().deleteItem(slot, item);
	for (int itemId : set.items)
	    player.getInventory().addItem(itemId, 1);
	player.packets().sendMessage("You sucessfully traded your item components for a set!");
    }

    public static void exchangeSet(Player player, int id) {
	Sets set = getSet(id);
	if (set == null) {
	    player.packets().sendMessage("This isn't a set item.");
	    return;
	}
	for (int itemId : set.items) {
	    if (!player.getInventory().containsItem(itemId, 1)) {
		player.packets().sendMessage("You don't have the parts to make up this set.");
		return;
	    }
	}
	for (int itemId : set.items)
	...
	player.packets().sendMessage(ItemExamines.getExamine(new Item(id, 1)));
    }
	...
	player.interfaces().sendInterface(645);
	player.interfaces().sendInventoryInterface(644);
	player.packets().sendIComponentSettings(645, 16, 0, 115, 14);
	player.packets().sendUnlockIComponentOptionSlots(644, 0, 0, 27, 0, 1, 2);
	player.packets().sendInterSetItemsOptionsScript(644, 0, 93, 4, 7, "Components", "Exchange", "Examine");
	player.packets().sendRunScriptBlank(676);
    }
	...
	if (totalAmount <= 0 || totalAmount > Integer.MAX_VALUE) {
	    player.packets().sendMessage("Can't open pack, amount too big.");
	    return;
	}
	player.lock(1);
	player.getInventory().deleteItem(packItem, requestCount);
	player.getInventory().addItem(new Item(openedItem, (int)totalAmount));
	player.packets().sendMessage("You open the pack and receive " + totalAmount + " "+ItemDefinition.get(openedItem).getName().toLowerCase()+".");
    }
}
