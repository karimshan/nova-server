...
import org.nova.game.player.controlers.UnderGroundDungeon;
...
public class LightSource {
	...
	if (slot == -1)
	    return false;
	else if (!forceExtinguish && player.getControllerManager().getControler() != null && player.getControllerManager().getControler() instanceof UnderGroundDungeon) {
	    player.packets().sendMessage("You cannot extinguish the " + item.getName().toLowerCase() + " as you will not have a light source.");
	    return false;
	}
	player.getInventory().replaceItem(LIGHT_SOURCES[0][slot], item.getAmount(), itemSlot);
	player.packets().sendMessage("You extinguish the " + item.getName().toLowerCase() + ".");
	return true;
    }
	...
	player.packets().sendMessage("You light the " + item.getName().toLowerCase() + ".");
	return true;
    }
}
