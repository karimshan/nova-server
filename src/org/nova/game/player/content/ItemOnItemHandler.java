	...
	public static void handleItemOnItem(Player player, ItemOnItem itemOnItem, int usedWith, int itemUsed) {
		...
		if (player.getSkills().getLevel(skillStuff[0]) >= skillStuff[1]) {
			if (player.getSkills().getLevel(skillStuff[3]) >= skillStuff[4]) {
				...
				player.packets().sendMessage("You need a " + Skills.SKILL_NAME[skillStuff[3]] + " level of " + Skills.SKILL_NAME[skillStuff[4]] + " to make this.");
		} else
			player.packets().sendMessage("You need a " + Skills.SKILL_NAME[skillStuff[0]] + " level of " + skillStuff[1] + " to make this.");
	}
