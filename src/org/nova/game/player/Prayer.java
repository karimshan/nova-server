...
public class Prayer implements Serializable {
	...
	public void increaseTurmoilBonus(Player p2) {
		leechBonuses[8] = (int) ((100 * Math.floor(0.15 * p2.getSkills()
				.getLevelFromXP(Skills.ATTACK))) / p2.getSkills().getLevelFromXP(
						Skills.ATTACK));
		leechBonuses[9] = (int) ((100 * Math.floor(0.15 * p2.getSkills()
				.getLevelFromXP(Skills.DEFENCE))) / p2.getSkills()
				.getLevelFromXP(Skills.DEFENCE));
		leechBonuses[10] = (int) ((100 * Math.floor(0.1 * p2.getSkills()
				.getLevelFromXP(Skills.STRENGTH))) / p2.getSkills()
				.getLevelFromXP(Skills.STRENGTH));
	...
	public void adjustStat(int stat, int percentage) {
		player.packets().sendConfigByFile(6857 + stat, 30 + percentage);
	}
	...
	public void switchSettingQuickPrayer() {
		if (player.isRemovedPrayers())
			return;
		usingQuickPrayer = !usingQuickPrayer;
		player.packets().sendGlobalConfig(181, usingQuickPrayer ? 1 : 0);// activates
		// quick
		// choose
		unlockPrayerBookButtons();
		if (usingQuickPrayer) // switchs tab to prayer
			player.packets().sendGlobalConfig(168, 6);
	}
	...
	public void switchQuickPrayers() {
		...
		if (hasPrayersOn())
			...
			if (hasOn) {
				player.packets().sendGlobalConfig(182, 1);
				recalculatePrayer();
			}
		}
	}
	...
	private boolean usePrayer(int prayerId) {
		if (player.isRemovedPrayers()) {
		player.sendMessage("You cannot use prayers during this battle.");
			return false;
		}
        if (player.isTransformed()) {
        player.sendMessage("You cannot activate any prayers while you're in a form.");
        	return false;
        }
    	if (getPrayerBook() == 0 && prayerId == 25 || prayerId == 27) {
			if (player.getSkills().getLevelFromXP(Skills.DEFENCE) < 70) {
				player.packets().sendMessage(
						"You need a defence level of at least 70 to use this prayer.");
				return false;
			}
    	}
			if (getPrayerBook() == 0 && prayerId == 25 || prayerId == 27 && !player.isCompletedHoarfrost()) {
				player.sendMessage("You must have completed Hoarfrost Hollow in order of use Piety.");
				return false;
			}
			if (getPrayerBook() == 0 && prayerId == 29 && !player.getLearnedPrayer()[1]) {
				player.sendMessage("I haven't learned this prayer yet.");
				return false;
			}
			if (getPrayerBook() == 0 && prayerId == 28 && !player.getLearnedPrayer()[0]) {
				player.sendMessage("I haven't learned this prayer yet.");
				return false;
			}
		if (prayerId < 0 || prayerId >= prayerLvls[getPrayerBook()].length)
			return false;
		if (player.getSkills().getLevelFromXP(5) < prayerLvls[this
			...
			if (player.getSkills().getLevelFromXP(Skills.DEFENCE) < 30) {
				player.packets().sendMessage(
		...
		if (!usingQuickPrayer) {
			if (onPrayers[getPrayerBook()][prayerId]) {
				...
				player.packets().sendSound(2663, 0, 1);
				return true;
			}
		} else {
		...
		player.packets().sendSound(2662, 0, 1);
		return true;
	}
	...
	public void resetDrainPrayer(int index) {
		nextDrain[index] = (long) (Misc.currentTimeMillis() + (prayerDrainRate[getPrayerBook()][index]*1000)
	...
	public void closeAllPrayers() {
		...
		onPrayersCount = 0;
		player.packets().sendGlobalConfig(182, 0);
		player.packets().sendConfig(ancientcurses ? 1582 : 1395, 0);
		player.getAppearance().generateAppearanceData();
	...
	private boolean checkPrayer() {
		if (prayerpoints <= 0) {
			player.packets().sendSound(2672, 0, 1);
			player.packets().sendMessage(
	...
	public void refresh() {
		player.packets().sendGlobalConfig(181, usingQuickPrayer ? 1 : 0);
		player.packets().sendConfig(1584, ancientcurses ? 1 : 0);
		unlockPrayerBookButtons();
	}
	...
	public void unlockPrayerBookButtons() {
		player.packets().sendUnlockIComponentOptionSlots(271,
	...
	public void refreshPrayerPoints() {
		player.packets().sendConfig(2382, prayerpoints);
	}
	...
	public boolean hasFullPrayerpoints() {
		return getPrayerpoints() >= player.getSkills().getLevelFromXP(Skills.PRAYER) * 10;
	}
	...
	public void restorePrayer(int amount) {
		int maxPrayer = player.getSkills().getLevelFromXP(Skills.PRAYER) * 10;
	...
	public void reset() {
		closeAllPrayers();
		prayerpoints = player.getSkills().getLevelFromXP(Skills.PRAYER) * 10;
		refreshPrayerPoints();
	}
	
	public boolean usingProtectMagic() {
		return usingPrayer(0, 17) || usingPrayer(1, 7);
	}
	
	public boolean usingProtectRange() {
		return usingPrayer(0, 18) || usingPrayer(1, 8);
	}
	
	public boolean usingProtectMelee() {
		return usingPrayer(0, 19) || usingPrayer(1, 9);
	}
