...
public class Auras implements Serializable {
	...
	public void process() {
		if (!isActivated())
			return;
		if (activation - Misc.currentTimeMillis() <= 60000 && !warned) {
			player.packets().sendMessage(
					"Your aura will deplete in 1 minute.");
			warned = true;
			return;
		}
		if (Misc.currentTimeMillis() < activation)
			return;
		desactive();
	...
	public void desactive() {
		activation = 0;
		warned = false;
		player.packets().sendMessage("Your aura has depleted.");
	}
	...
	public void activate() {
		...
		if (item.defs().containsOption("Discard"))
			return;
		...
		if (toId != -1) {
			...
			if (Misc.currentTimeMillis() <= getCoolDown(item.getId())) {
				// TODO message cooldown
				return;
			}
			int tier = getTier(item.getId());
			activation = Misc.currentTimeMillis()
	...
	public double getMagicAccurayMultiplier() {
		if (!isActivated() || Game.pvpArea(player))
			return 1;
		int aura = player.getEquipment().getAuraId();
		if (aura == 20962)
			return 1.03;
