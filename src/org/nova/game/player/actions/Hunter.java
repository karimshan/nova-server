...
public class Hunter extends Action {
	   ...
	   public enum FlyingEntities {
			...
			/* Steel platebody */, new Item(1636, 10), new Item(2135, 25) /*
				...
				if (Misc.getRandom(1) == 0) {
				    Item charm = CHARMS[Misc.random(CHARMS.length)];
				    int charmAmount = Misc.getRandom(charm.getAmount());
				    player.getMatrixDialogues().startDialogue("ItemMessage", "The impling was carrying a" + charm.getName().toLowerCase() + ".", charm.getId());
			...
			KINGLY_IMPLING(7903, 15517, 434, 617, 91, new Item[] { new Item(1705, Misc.random(3, 11)), new Item(1684, 3), new Item(1618, Misc.random(17, 34)), new Item(990, 2) }, new Item[] { new Item(1631, 1), new Item(1615, 1), new Item(9341, 40 + Misc.getRandom(30)), new Item(9342, 57), new Item(15511, 1), new Item(15509, 1), new Item(15505, 1), new Item(15507, 1), new Item(15503, 1), new Item(11212, 40 + Misc.random(104)), new Item(9193, 62 + Misc.random(8)), new Item(11230, Misc.random(182, 319)), new Item(11232, 70), new Item(1306, Misc.random(1, 2)), new Item(1249, 1) }, null, new Item[] { new Item(7158, 1), new Item(2366, 1), new Item(6571, 1) }),
	...
	public boolean start(Player player) {
		if (!checkAll(player))
			return false;
		player.packets().sendMessage("You start setting up the trap..");
	...
	public int processWithDelay(Player player) {
		...
		OwnedObjectManager.addOwnedObjectManager(player,
				new GlobalObject[] { new GlobalObject(hunt.getObjectId(), 10, 0,
	...
	private boolean checkAll(Player player) {
		...
		List<GlobalObject> objects = Game.getRegion(player.getRegionId())
				.getSpawnedObjects();
		if (objects != null) {
			for (GlobalObject object : objects) {
				if (object.getX() == player.getX()
						&& object.getY() == player.getY()
						&& object.getZ() == player.getZ()) {
					player.packets().sendMessage(
