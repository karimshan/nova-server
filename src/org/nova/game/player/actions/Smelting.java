...
public class Smelting extends Action {
	...
	public Smelting(int slotId, GlobalObject object, int ticks) {
		this.object = object;
	...
	public boolean start(Player player) {
		...
		if (!player.getInventory().containsItem(
			...
			player.getMatrixDialogues().startDialogue(
					"SimpleMessage",
					"You need "
							+ bar.getItemsRequired()[0].defs()
									.getName() + " to create a "
							+ bar.getProducedBar().defs().getName()
							+ ".");
			return false;
		}
		if (bar.getItemsRequired().length > 1) {
			if (!player.getInventory().containsItem(
				...
				player.getMatrixDialogues().startDialogue(
						"SimpleMessage",
						"You need "
								+ bar.getItemsRequired()[1].defs()
										.getName()
								+ " to create a "
								+ bar.getProducedBar().defs()
										.getName() + ".");
				return false;
			}
		}
	...
	public boolean isSuccessFull(Player player) {
		if (bar == SmeltingBar.IRON) {
			if (player.getEquipment().getItem(Equipment.SLOT_RING) != null
				...
				return Misc.getRandom(100) <= (player.getSkills().getLevel(
