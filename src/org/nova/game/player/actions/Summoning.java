...
public class Summoning implements Serializable {
    ...
    public static void sendInterface(Player player) {
        ...
        player.packets().sendRunScript(757, options);
        player.packets().sendIComponentSettings(INTERFACE, 16, 0, 462, 190);
    }

    public static Familiar createFamiliar(Player player, Pouches pouch) {
        String loc = "org.nova.game.npc.familiar.";
        try {
            Familiar fam = (Familiar) Class.forName(
                    loc
                    + NPCDefinition.get(pouch.getNpcId()).name.replace(" ", "").replace("-", "")).getConstructor(
                    new Class[]{Player.class, Pouches.class,
                        Location.class, Integer.TYPE, Boolean.TYPE}).newInstance(
	...
	public static void spawnFamiliar(Player player, Pouches pouch) {
		if (player.getFamiliar() != null || player.getRealPet() != null) {
			player.packets().sendMessage("You already have a follower.");
			return;
		}
		if (!player.getControllerManager().canSummonFamiliar()) 
			return;
		ItemDefinition def = ItemDefinition.get(pouch
				.getPouchId());
		if (def == null)
			return;
		HashMap<Integer, Integer> skillReq = def.getWearingSkillRequirements();
    ...
    public static void createPouch(final Player player, int itemId, int amount) {
        SummoningPouch pouch = SummoningPouch.get(itemId);
        if (pouch == null) {
            player.packets().sendMessage("You do not have the items required to create this pouch.");
            return;
        }
        ...
        for (i = 0; i < amount; i++) {
            ...
            player.packets().sendObjectAnimation(
                    new GlobalObject(28716, 10, 1, 2209, 5344, 0),
                    new Animation(8509));
        }
        if (i == 1) {
            player.packets().sendMessage("You infuse a "
                    + ItemDefinition.get(itemId).getName().toLowerCase() + ".");
        } else if (i > 0) {
            player.packets().sendMessage("You infuse some "
                    + ItemDefinition.get(itemId).getName().toLowerCase() + "es.");
        } else {
            return;
        }
	...
	public static void infusePouches(Player player) {
		player.interfaces().sendInterface(INTERFACE);// close to this
		...
		player.packets().sendRunScript(757, options);
		player.packets().sendIComponentSettings(INTERFACE, 16, 0, 430, 190);
	}
	public static void attackDreadnipTarget(NPC target, Player player) {
		if (target.isDead()) {
			player.packets().sendMessage("Your target is already dead!");
			return;
	...
	public static void openDreadnipInterface(Player player) {
		player.interfaces().closeInventory();
		player.interfaces().sendTab(player.interfaces().isFullScreen() ? 115 : 175, 1165);
	}
