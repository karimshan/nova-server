...
public class Smithing extends Action {
	...
	public static class ForgingInterface {
		...
		public static String[] getStrings(Player player, ForgingBar bar,
			...
			String name = ItemDefinition.get(itemId).getName()
					.toLowerCase();
			...
			levelString.append(Misc.formatPlayerNameForDisplay(name.replace(
					barVariableName + " ", "")));
		...
		private static void sendComponentConfigs(Player player, ForgingBar bar) {
			for (int i : bar.getComponentChilds()) {
				player.packets().sendHideIComponent(SMITHING_INTERFACE,
						i - 1, false);
			}
		}
		...
		public static void sendSmithingInterface(Player player, ForgingBar bar) {
			...
			player.packets().sendIComponentText(
					300,
					14,
					Misc.formatPlayerNameForDisplay(bar.toString()
							.toLowerCase()) + "");
			player.interfaces().sendInterface(SMITHING_INTERFACE);
		}
	}
}
