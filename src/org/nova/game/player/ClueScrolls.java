...
public class ClueScrolls {
	//chest id 2717
	public static int[] ScrollIds = {2677,2678,2679,2680,2681,2682,2683,2684,2685,2686,2687,2688,2689,2690,2691,2692,2693,
	...
	private enum Riddles {
		Riddle1(20, new int[] {2967,4386,2970,4380}, new String[] {
				"There once was a villan","of grey and white", "he also had a bit of bage","do a clap outside his cave",
	...
	public static boolean completedRiddle(Player p, int emoteid){
		...
		if (hasRiddleClue(p, scroll.id) != null){
			...
			Location lastloc = p.getLastLocation();
	...
	public static boolean objectSpot(Player p, GlobalObject obj){
		Scrolls scroll = hasClue(p);
		if (scroll != null){
	...
	public static void showObjectMap(Player p, ObjectMaps objmap){
		p.packets().sendInterface(false, p.interfaces().isFullScreen() ? 746:548, p.interfaces().isFullScreen() ? 28:27, objmap.interid);
		p.sm(objmap.hint);
		
	}
	
	public static void showRiddle(Player p, Riddles riddle){
		p.packets().sendInterface(false, p.interfaces().isFullScreen() ? 746:548, p.interfaces().isFullScreen() ? 28:27, 345);
		p.packets().sendIComponentText(345, 1, riddle.riddles[0]);
		p.packets().sendIComponentText(345, 2, riddle.riddles[1]);
		p.packets().sendIComponentText(345, 3, riddle.riddles[2]);
		p.packets().sendIComponentText(345, 4, riddle.riddles[3]);
		p.packets().sendIComponentText(345, 5, riddle.riddles[4]);
		p.packets().sendIComponentText(345, 6, riddle.riddles[5]);
		p.packets().sendIComponentText(345, 7, riddle.riddles[6]);
		p.packets().sendIComponentText(345, 8, riddle.riddles[7]);
	}
	
	public static void showMap(Player p, Maps map){
		p.packets().sendInterface(false, p.interfaces().isFullScreen() ? 746:548, p.interfaces().isFullScreen() ? 28:27, map.interfaceId);
		p.sm(map.chat);
	}
	...
	public static void giveReward(Player p){
		int random = Misc.getRandom(999)+1;
		if (p.cluenoreward == 1){
			random += 100;
			p.cluenoreward += 1;
		...
		if (random > 875){
			Item[] rewards;
			if (Misc.getRandom(500) > 475 && Misc.getRandom(999) > 950){
			rewards = new Item[] {EasyRewards[Misc.getRandom(EasyRewards.length)],EasyRewards[Misc.getRandom(EasyRewards.length)],EasyRewards[Misc.getRandom(EasyRewards.length)],ThirdAge[Misc.getRandom(ThirdAge.length)]};
				
			} else {
			rewards = new Item[] {EasyRewards[Misc.getRandom(EasyRewards.length)],EasyRewards[Misc.getRandom(EasyRewards.length)],EasyRewards[Misc.getRandom(EasyRewards.length)]};
			}
			...
			p.getInventory().deleteItem(2717,1);
			p.getInventory().addItem(ScrollIds[Misc.getRandom(ScrollIds.length)], 1);
			if (p.cluenoreward == 0){
				p.cluenoreward += 1;
			}
