...
public class MrEx extends MatrixDialogue {
		...
		sendEntityDialogue(SEND_2_TEXT_CHAT,
				new String[] { NPCDefinition.get(npcId).name,
						"Hey there, "+player.getDisplayName()+".",
								"You can call me Mr Ex." }, IS_NPC, npcId, 9827);
	}
	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			...
			sendDialogue(SEND_5_OPTIONS, "What you want to do?", 
					"I want Wildstaker helmet.", "I have lost my Wildstaker helmet. May I have one?", "Nevermind.", "", "");
			stage = 2;
		} 
		else if (stage == 2) {
			if(componentId == 1) {
			WildernessHelmets.giveFirstTimeHat(player);
			}
			else if(componentId == 2) {
				WildernessHelmets.checkLostHat(player);
			}
