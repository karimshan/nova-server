...
public class TaskD extends MatrixDialogue {
		...
		sendEntityDialogue(SEND_2_TEXT_CHAT,
				new String[] { NPCDefinition.get(npcId).name,
						"Hey there, "+player.getDisplayName()+".",
								" are you looking for a task or maybe quest capes?" }, IS_NPC, npcId, 9827);
	}
	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			...
			sendDialogue(SEND_5_OPTIONS, "What you want to do?", 
					"I want to talk about the tasks system.", "I want a quest points cape.", "Nevermind.", "", "");
			stage = 2;
		} 
		else if (stage == 2) {
			if(componentId == 1) {
				...
				//QuestManager.checkforQuestcape(player);
			}
