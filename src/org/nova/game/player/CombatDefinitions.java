...
public final class CombatDefinitions implements Serializable {
	...
	public void refreshAutoCastSpell() {
		refreshAttackStyle();
		player.packets().sendConfig(108, getSpellAutoCastConfigValue());
	}
	 ...
	 public static final int getXpStyle(int weaponId, int attackStyle) {
			if (weaponId != -1 && weaponId != -2) {
			    ...
			    if(weaponName.contains("the first blade") 
			    		|| weaponName.contains("aeon sword") || weaponName.contains("assassin's blade")) {
			    	switch (attackStyle) {
				    case 0:
	...
	String name = weaponId == -1 ? "" : ItemDefinition.get(weaponId).getName().toLowerCase();
	...
	public void refreshAttackStyle() {
		player.packets().sendConfig(43, autoCastSpell > 0 ? 4 : attackStyle);
	}

	public void sendUnlockAttackStylesButtons() {
		for (int componentId = 11; componentId <= 14; componentId++)
			player.packets().sendUnlockIComponentOptionSlots(884,
					componentId, -1, 0, 0);
	}
	...
	public void refreshUsingSpecialAttack() {
		player.packets().sendConfig(301, usingSpecialAttack ? 1 : 0);
	}

	public void refreshSpecialAttackPercentage() {
		player.packets().sendConfig(300, specialAttackPercentage * 10);
	}
	...
	public void refreshAutoRelatie() {
		player.packets().sendConfig(172, autoRelatie ? 0 : 1);
	}
	...
	public void setAutoRetaliate(boolean autoRelatie) {
	...
	public void restoreSpecialAttack() {
		...
		if(specialAttackPercentage == 100 || specialAttackPercentage == 50)
			player.packets().sendMessage("<col=00FF00>Your special attack energy is now "+specialAttackPercentage+"%.", true);
	}

	public void restoreSpecialAttack(int percentage) {
		if (specialAttackPercentage >= 100
				|| player.interfaces().containsScreenInter())
			return;
