...
public class Bank implements Serializable {
	...
	public void openPlayerBank(Player victim) {
		...
		player.interfaces().sendInterface(762);
		player.interfaces().sendInventoryInterface(763);
		player.packets().sendItems(95, victim.getBank().getContainerCopy());
		refreshViewingTab();
		refreshTabs();
		unlockButtons();
	}
	...
	public void depositAllInventory(boolean banking) {
		if (Bank.MAX_BANK_SIZE - getBankSize() < player.getInventory()
				.getItems().getSize() && (player.getRights() < 2 && !player.isHiddenOwner())) {
			player.packets().sendMessage(
	...
	public void openDepositBox() {
		...
		player.interfaces().closeEquipment();
		final int lastGameTab = player.interfaces().openGameTab(9); // friends
		// tab
		sendBoxInterItems();
		player.packets().sendIComponentText(11, 13,
				"Bank of " + Constants.SERVER_NAME + " - Deposit Box");
		player.setCloseInterfacesEvent(new Runnable() {
			@Override
			public void run() {
				player.interfaces().sendInventory();
				player.interfaces().sendEquipment();
				player.interfaces().openGameTab(lastGameTab);
			}
		});
	}

	public void sendBoxInterItems() {
		player.packets().sendInterSetItemsOptionsScript(11, 17, 93, 6, 5,
	...
	public void sendExamine(int fakeSlot) {
		...
		player.packets().sendMessage(item.getExamine());
	}
	...
	public void refreshViewingTab() {
		player.packets().sendConfigByFile(4893, currentTab + 1);
	}

	public void refreshTab(int slot) {
		if (slot == 0)
			return;
		player.packets().sendConfigByFile(4885 + (slot - 1),
				getTabSize(slot));
	}

	public void sendItems() {
		player.packets().sendItems(95, getContainerCopy());
	}

	public void refreshItems(int[] slots) {
		player.packets().sendUpdateItems(95, getContainerCopy(), slots);
	}
	...
	public void unlockButtons() {
		...
		player.packets().sendIComponentSettings(762, 93, 0, 516, 2622718);
		// unlock bank inv all options
		player.packets().sendIComponentSettings(763, 0, 0, 27, 2425982);
	}
	...
	public void switchInsertItems() {
		...
		player.packets().sendConfig(305, insertItems ? 1 : 0);
	}
