...
public class DuelConfigurations {
	 ...
	 */
	public transient boolean[] duelRules = new boolean[26];

	/**
	 * Duel arena lobby location.
	 */
	public Location DUELARENA_LOBBY = new Location(3365, 3275, 0);

	/**
	 * Player's stake.
	 */
	private ItemsContainer<Item> stake = new ItemsContainer<Item>(28, false);

	/**
	 * Target's stake.
	 */
	private ItemsContainer<Item> stake2 = new ItemsContainer<Item>(28, false);
	 ...
	 *            is stake.
	 */
	public DuelConfigurations(final Player player, final Player target,
			boolean ifFriendly) {
		this.target = target;
		this.player = player;
		...
		target.setCloseInterfacesEvent(new Runnable() {
			@Override
			public void run() {
				declineDuel(target);
			}
	 ...
	 *            the player
	 * @param ifFriendly
	 *            {@code true} if the duel is friendly, {@code false} duel is
	 *            stake
	 */
	private void updateScreen(Player player, boolean ifFriendly) {
		...
		player.packets().sendItems(134, false, stake);
		player.packets().sendItems(134, true, stake2);
		player.packets().sendItems(94, false,
				player.getInventory().getItems());
		player.packets().sendIComponentText(
				ifFriendly ? 637 : 631,
				ifFriendly ? 16 : 38,
				" "
						+ Misc.formatPlayerNameForDisplay(getOther(player)
								.getUsername()));
		player.packets().sendIComponentText(ifFriendly ? 637 : 631,
				ifFriendly ? 18 : 40,
				"" + (getOther(player).getSkills().getCombatLevel()));
		player.packets().sendConfig(286, 0);
		player.getTemporaryAttributtes().put("firstScreen", true);
		player.interfaces().sendInterface(ifFriendly ? 637 : 631);
	}
	 ...
	 * 
	 * @param player
	 *            The player whoes duel is going to end.
	 * @param declined
	 *            {@code true} if it's declined, {@code false} if not.
	 * @param logout
	 *            {@code true} if logged out, {@code false} if not.
	 */
	public void endDuel(Player player, boolean declined, boolean logout) {
		...
		if (!declined) {
			if (logout)
				player.setCoords(DUELARENA_LOBBY);
			else
				player.setLocation(DUELARENA_LOBBY);
		} else {
	 ...
	 * 
	 * @param player
	 *            The player to whom options are going to send.
	 */
	public void sendInventory(Player player) {
		player.interfaces().sendInventoryInterface(628);
		player.packets().sendItems(93, player.getInventory().getItems());
		player.packets().sendUnlockIComponentOptionSlots(628, 0, 0, 27, 0,
				1, 2, 3, 4, 5);
		player.packets().sendInterSetItemsOptionsScript(628, 0, 93, 4, 7,
				"Stake 1", "Stake 5", "Stake 10", "Stake All", "Examine");
		player.packets().sendUnlockIComponentOptionSlots(631, 103, 0, 27, 0,
				1, 2, 3, 4, 5);
		player.packets().sendInterSetItemsOptionsScript(631, 0, 134, 4, 7,
	 ...
	 */
	public boolean processButtonClick(Player player, int interfaceId,
		...
		case 631:
			...
			switch (packetId) {
			case PacketDecoder.ACTION_BUTTON1_PACKET:
				removeStake(player, slotId, 1);
				return false;
			case PacketDecoder.ACTION_BUTTON2_PACKET:
				removeStake(player, slotId, 5);
				return false;
			case PacketDecoder.ACTION_BUTTON3_PACKET:
				removeStake(player, slotId, 10);
				return false;
			case PacketDecoder.ACTION_BUTTON4_PACKET:
				Item item = player.getInventory().getItems().get(slotId);
				if (item == null)
					return false;
				removeStake(player, slotId, player.getInventory().getItems()
						.getNumberOf(item));
				return false;
			case PacketDecoder.ACTION_BUTTON5_PACKET:
		...
		case 628:
			...
			case PacketDecoder.ACTION_BUTTON4_PACKET:
				...
				if (item == null)
					return false;
				offerStake(player, slotId, player.getInventory().getItems()
						.getNumberOf(item));
				return false;
			case PacketDecoder.ACTION_BUTTON5_PACKET:
				player.getInventory().sendExamine(slotId);
	 ...
	 * 
	 * @param player
	 *            The player who declined the duel.
	 */
	private void declineDuel(Player player) {
		Player other = getOther(player);
		other.setCloseInterfacesEvent(null);
		player.setCloseInterfacesEvent(null);
		player.packets().sendMessage("You've declined the duel.");
		other.packets().sendMessage(
	 ...
	 * 
	 * @param player
	 *            The player who's accepting the duel.
	 * @param ifFriendly
	 *            {@code true} if the duel is friendly, {@code false} the duel
	 *            is stake.
	 */
	public void handleAccept(final Player player, boolean ifFriendly) {
		if (canAccept(player)) {
			Player other = getOther(player);
			player.getTemporaryAttributtes().put("accepted", true);
			other.packets().sendIComponentText(ifFriendly ? 637 : 631,
					ifFriendly ? 20 : 41, "Other player has accepted.");
			player.packets().sendIComponentText(ifFriendly ? 637 : 631,
					ifFriendly ? 20 : 41, "Waiting for other player...");
			if (other.getTemporaryAttributtes().get("accepted") != null) {
				player.setCloseInterfacesEvent(null);
				other.setCloseInterfacesEvent(null);
				player.getTemporaryAttributtes().put("accepted", false);
				other.getTemporaryAttributtes().put("accepted", false);
				player.getTemporaryAttributtes().remove("firstScreen");
				other.getTemporaryAttributtes().remove("firstScreen");
				openSecondInterface(player, other, ifFriendly);
				openSecondInterface(other, player, ifFriendly);
				if (stake.getFreeSlot() != 0) {
					player.packets().sendIComponentText(626, 25, "");
					other.packets().sendIComponentText(626, 26, "");
				}
				if (stake2.getFreeSlot() != 0) {
					player.packets().sendIComponentText(626, 26, "");
					other.packets().sendIComponentText(626, 25, "");
				}
			}
		}
	}
	...
	public void handleSecondScreenAccept(final Player player, boolean ifFriendly) {
		...
		player.getTemporaryAttributtes().put("accepted", true);
		other.packets().sendIComponentText(ifFriendly ? 639 : 626,
				ifFriendly ? 23 : 35, "Other player has accepted.");
		player.packets().sendIComponentText(ifFriendly ? 639 : 626,
				ifFriendly ? 23 : 35, "Waiting for other player...");
		if (other.getTemporaryAttributtes().get("accepted") == Boolean.TRUE) {
			player.setCloseInterfacesEvent(null);
			other.setCloseInterfacesEvent(null);
			player.getTemporaryAttributtes().put("accepted", false);
			other.getTemporaryAttributtes().put("accepted", false);
	...
	private void teleportPlayer(Player player, Player other) {
		...
		player.setLocation(random == 0 ? teleports[0] : teleports[1]);
		other.setLocation(random == 0 ? teleports[1] : teleports[0]);
	 ...
	 * 
	 * @param player
	 *            The player who's equipment is going to be removed.
	 */
	private void removeEquipment(Player player) {
		...
		for (int i = 10; i < 24; i++) {
			if (getRule(i)) {
				slot = i - 10;
				ButtonPackets.sendRemove(player, slot);
			}
		}
	}
	...
	private Location[] getArenaTeleport() {
		final int arenaChoice = Misc.getRandom(2);
		...
		int finalX = arenaBoundariesX[arenaChoice]
				+ Misc.getRandom(maxOffsetX[arenaChoice]);
		int finalY = arenaBoundariesY[arenaChoice]
				+ Misc.getRandom(maxOffsetY[arenaChoice]);
		locations[0] = (new Location(finalX, finalY, 0));
		if (getRule(25)) {
			int direction = Misc.getRandom(1);
			if (direction == 0) {
				finalX--;
			} else {
				finalY++;
	...
	private boolean canAccept(Player player) {
		...
		for (int i = 10; i < 24; i++) {
			...
			if (getRule(i)
				...
				if (i == 23) {// arrows
					if (!(item.defs().isStackable() && player
	 ...
	 *            is stake.
	 */
	private void openSecondInterface(Player player, Player other,
			boolean ifFriendly) {
		other.interfaces().sendInterface(ifFriendly ? 639 : 626);
		player.interfaces().sendInterface(ifFriendly ? 639 : 626);
	}
	...
	private void setRules(int ruleId) {
		...
		player.getTemporaryAttributtes().remove("accepted");
		target.getTemporaryAttributtes().remove("accepted");
		setConfigs();
	}
	...
	private void setConfigs() {
		...
		player.packets().sendConfig(286, value);
		target.packets().sendConfig(286, value);
	}
	 ...
	 * 
	 * @param p
	 *            The player who's offering.
	 * @param slot
	 *            The slot.
	 * @param amount
	 *            The amount to be offerd.
	 */
	private void offerStake(Player p, int slot, int amount) {
		...
		if (!ItemConstants.isTradeable(item)) {
			p.packets().sendMessage("That item isn't stakeable.");
			return;
		}
		player.getTemporaryAttributtes().remove("accepted");
		target.getTemporaryAttributtes().remove("accepted");
	...
	public void refresh(int... slots) {
		player.packets().sendUpdateItems(134, stake, slots);
		player.packets().sendUpdateItems(-134, stake2, slots);
		target.packets().sendUpdateItems(134, stake, slots);
		target.packets().sendUpdateItems(-134, stake2, slots);
		player.packets().sendUpdateItems(93,
				player.getInventory().getItems(), slots);
		target.packets().sendUpdateItems(93,
				target.getInventory().getItems(), slots);
		player.getInventory().refresh();
		target.getInventory().refresh();
	}

	/**
	 * Rewaring of the stakes.
	 * 
	 * @param player
	 *            The winner.
	 */
	...
	public void addSpoils(Player player) {
		Player other = getOther(player);
		other.interfaces().sendInterface(634);
		if (player == this.player)
			other.packets().sendItems(136, false, stake);

		if (player == target)
			other.packets().sendItems(136, false, stake2);

		other.packets().sendUnlockIComponentOptionSlots(634, 28, 0, 35, 0,
				1, 2, 3, 4, 5);
		other.packets().sendInterSetItemsOptionsScript(634, 28, 136, 4, 7,
				"", "", "", "", "");
		other.packets().sendIComponentText(634, 23,
				Misc.formatPlayerNameForDisplay(player.getUsername()));
		other.packets().sendIComponentText(634, 22,
