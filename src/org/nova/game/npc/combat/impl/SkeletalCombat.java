...
public class SkeletalCombat extends CombatScript {
 private Player player;
	@Override
	public Object[] getKeys() {
		return new Object[] { 9177 };
	}
	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (Misc.getRandom(10) == 0) {
			...
			npc.heal(500);
			player.sm("Skeletal Horror has used special skill, he's constitution was healed by 500 lifepoints.");
		}
		if (attackStyle == 0) {
			npc.setNextAnimation(new Animation(12061));
			delayHit(
					npc,
					2,
					target,
			...
			npc.setNextAnimation(new Animation(12060));
			delayHit(
					npc,
					2,
					target,
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				@Override
				public void run() {
					target.setNextGraphics(new Graphics(1891));
				}
			...
			npc.setNextAnimation(new Animation(12065));
			...
			npc.setNextAnimation(new Animation(12062));
			npc.setNextGraphics(new Graphics(2618));
			target.setNextGraphics(new Graphics(2617));
