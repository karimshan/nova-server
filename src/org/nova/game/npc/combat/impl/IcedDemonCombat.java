...
public class IcedDemonCombat extends CombatScript {
		...
		if (attackStyle == 0) {
			...
			if (Combat.hasAntiDragProtection(target)) {
				...
				if (player != null)
					player.packets().sendMessage(
							"You are hit by the Ice Demon's poisonous breath!",
							true);
			}
			...
			if (Combat.hasAntiDragProtection(target)) {
				...
				if (player != null)
					player.packets()
							.sendMessage(
									"Your shield absorbs most of the Ice Demon's shocking breath!",
									true);
			} else if (player != null
					&& (player.getPrayer().usingPrayer(0, 17) || player
							.getPrayer().usingPrayer(1, 7))) {
				...
				if (player != null)
					player.packets()
							.sendMessage(
									"Your prayer absorbs most of the Ice Demon's shocking breath!",
									true);
			} else {
				damage = Misc.getRandom(650);
				if (player != null)
